...
if __FILE__ == $0
  ...
  ARGV.each do |i|
    ...
    if j == 0
      ...
      if i == 'deltaQ'
        csv_data = CSV.read("#{dir}/#{i}.log", headers: true, col_sep: " ")
      else
        csv_data = CSV.read("#{dir}/thread_#{i}.log", headers: true, col_sep: " ")
      end
      params_ary << csv_to_dataset(csv_data, i, func)
    end
    j = j + 1
  end
  ...
  #  end
  Gnuplot.open do |gp|
    gnu_plot(gp, params_ary, "node_number", "elapsed", func, "number of node", "elapsed[s]")
  end
  #  Gnuplot.open do |gp|
