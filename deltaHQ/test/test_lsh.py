...
class LSHTester:
    ...
    def run(self, name, metric, hash_family, k_vals, L_vals, radius):
        ...
        #  print(name)
        #  print('L\tk\tacc\ttouch')
        print('L\tk\tloop_ave\trate_avg\telapsed_avg\tdiff_loop_avg')
        for k in k_vals:  # concatenating more hash functions increases selectivity
            ...
            for L in L_vals:  # using more hash tables increases recall
                ...
                for p in points:
                    ...
                    mod_loop += ori_candi_len - len(exact_points)
                ...
                print("{0}\t{1}\t{2:.2f}\t\t{3:.5f}\t\t{4:.9f}\t{5}".format(
                    L,
                    k,
                    sum_loop / points_len,
                ...
                print("log {0} {1}".format(len(points), sum_loop / points_len))
            print()
...
if __name__ == "__main__":
    ...
    test = [12, 256, 512, 1024, 2048, 4096, 8192]

    for i in range(1):
        num_points = test[0] * 2**i
        print("num: {0}\n".format(num_points))
        xmax = int((num_points * 10**6 / 341)**0.5)
        #  print(xmax)
        points = [
        ...
        args = {
            'name': 'L2',
            'metric': lsh.L2_norm,
            'hash_family': lsh.L2HashFamily(radius * 1.05, d),
