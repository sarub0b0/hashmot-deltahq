...
#define FLOAT_CMP(a, b) (fabs(a - b) < FLT_EPSILON)
...
void environment_init(struct environment_class *environment,
                      const char *name,
                      const char *type,
                      double alpha,
                      double sigma,
...
int segment_intersect(double xn1,
                      double yn1,
                      double xn2,
                      double yn2,
                      double xo1,
    ...
    double denominator;
    /*
       DEBUG("Intersect [(%.2f,%.2f),(%.2f,%.2f)] and
       [(%.2f,%.2f),(%.2f,%.2f)]:",
    ...
    if (fabs(ro1) >= EPSILON && fabs(ro2) >= EPSILON &&
        SAME_SIGNS(ro1, ro2)) {
        // DEBUG("\tno intersection (ro1=%f, ro2=%f have same sign)", ro1,
        // ro2);
        return FALSE;
    }
    ...
    if (fabs(rn1) >= EPSILON && fabs(rn2) >= EPSILON &&
        SAME_SIGNS(rn1, rn2)) {
        // DEBUG("\tno intersection (rn1=%f, rn2=%f have same sign)", rn1,
        // rn2);
        return FALSE;
    }
...
int point_in_object(
    double xn, double yn, double xo1, double yo1, double xo2, double yo2) {
...
int point_on_object_edge2d(double xn,
                           double yn,
...
// return TRUE if point (xn,yn) is in object (xo1,yo1)x(xo2,yo2)
int point_in_object3d(double xn,
                      double yn,
                      double zn,
                      struct object_class *object) {
    ...
    if (zn > object->height) return FALSE;
...
int internal_point_on_object_edge(
    double xn, double yn, double xo1, double yo1, double xo2, double yo2) {
...
int segment_in_object(double xn1,
                      double yn1,
                      double xn2,
                      double yn2,
                      double xo1,
...
// included in object (xo1,yo1)x(xo2,yo2)
int segment_in_object3d(double xn1,
                        double yn1,
                        double zn1,
                        double xn2,
                        double yn2,
...
void intersection_print(double *intersections_x,
...
int intersection_do_insert(double *intersections_x,
                           double *intersections_y,
                           int *number,
                           double new_x,
                           double new_y,
...
// return SUCCESS on success, ERROR on error
int intersection_add_point(double *intersections_x,
                           double *intersections_y,
                           int *number,
                           double new_x,
                           double new_y) {
    ...
    if ((*number) == 0) {
        intersection_do_insert(
            intersections_x, intersections_y, number, new_x, new_y, 0);
    ...
    for (ix = 0; ix < (*number); ++ix) {
        ...
        if (FLOAT_CMP(intersections_x[ix], new_x)) {
            ...
            for (iy = ix; iy < (*number); ++iy)
                ...
                // if y coordinate is larger, insert as previous item
                    ...
                    intersection_do_insert(intersections_x,
                                           intersections_y,
                                           number,
                                           new_x,
                                           new_y,
        ...
        // if x coordinate is larger, insert as previous item
            ...
            intersection_do_insert(
                intersections_x, intersections_y, number, new_x, new_y, ix);
            return SUCCESS;
        }
    }
...
int environment_update(struct environment_class *environment,
    ...
    intersection_add_point(intersections_x,
                           intersections_y,
                           &number,
    ...
    if (number == 1) {
        WARNING(
            "No intersections found. Possbile node overlap. \
                ...
                if (height_check_needed == TRUE) {
                    double min_height =
                        (from_node->position.c[2] < to_node->position.c[2])
                            ? from_node->position.c[2]
                            : to_node->position.c[2];
                    double height_diff  = fabs(from_node->position.c[2] -
                                              to_node->position.c[2]);
                    double total_length = sqrt(
                        pow(from_node->position.c[0] - to_node->position.c[0],
                            2) +
                        pow(from_node->position.c[1] - to_node->position.c[1],
                            2));
                    double angle = atan(height_diff / total_length);

                    double min_height_intersection1, min_height_intersection2;
                    double distance_to_intersection1,
                        distance_to_intersection2;

                    DEBUG(
                        "from_node<->to_node (I): \
total_length=%f height_diff=%f angle=%f",
                        total_length,
                        height_diff,
                        angle);
...
min_height_intersection2=%f",
                    ...
                    if (min_height_intersection1 >
                        ...
                        min_height_intersection2 >
                            scenario->objects[object_index].height) {
                        DEBUG(
                            "Ignored intersection point due to height \
differences");
                        continue;
                    }
                }
        ...
        for (object_index = 0; object_index < scenario->object_number;
            ...
            if (segment_in_object3d(intersections_x[i],
                ...
                // determines the segment properties
                    ...
                    printf(
                        "Check whether new intersection object %d is \
included in the previously detected intersection object %d\n",
                    ...
                    for (vertex_i = 0;
                        ...
                        if (point_on_object_edge2d(
                                ...
                                scenario->objects[object_index]
                                    ...
                                    .c[1],
                                &(scenario
                                      ->objects[intersections_objects[i]]))) {
                            printf(
                                "Vertex %d of object %d IS included in "
                                "object "
                                "%d (EDGE)\n",
                                vertex_i,
                                object_index,
                    ...
                    {
                        DEBUG(
                            "Latest object is included in the previously \
found one => update inclusion information");
    ...
    if (FLOAT_CMP(intersections_x[0], from_node->position.c[0]) &&
        FLOAT_CMP(intersections_y[0], from_node->position.c[1])) {
        // we start from index 0 and go towards the end
        // increasing = TRUE;
    } else if (FLOAT_CMP(intersections_x[0], to_node->position.c[0]) &&
               FLOAT_CMP(intersections_y[0], to_node->position.c[1])) {
    ...
    for (i = 0; i < number - 1; ++i) {
        ...
        if (length < EPSILON) {
            ...
            if (fabs(from_node->position.c[2] - to_node->position.c[2]) <
                ...
                DEBUG(
                    "from_node<->to_node (II): total_length=%f \
height_diff==%f angle=%f",
                    total_length,
                    height_diff,
                    angle);
...
((%.2f,%.2f)<->(%.2f,%.2f)...\n",
                    ...
                    if (point_on_segment(intersections_x[i],
                                         intersections_y[i],
                                         object->vertices[vertex_i].c[0],
                                         object->vertices[vertex_i].c[1],
                                         object->vertices[vertex_i2].c[0],
                    ...
                    printf("Check for intersection point (%.2f,%.2f)\n",
                           intersections_x[i + 1],
                           intersections_y[i + 1]);
                    if (point_on_segment(intersections_x[i + 1],
                                         intersections_y[i + 1],
                                         object->vertices[vertex_i].c[0],
                                         object->vertices[vertex_i].c[1],
                                         object->vertices[vertex_i2].c[0],
...
Using wall attenuation x 1");
                ...
                {
                    DEBUG(
                        "Segment has both ends on object edges. \
Using wall attenuation x 2");
            ...
            WARNING(
                "No environment information found. Using default \
...
// of the change, and since it is a decrease, it should be subtracted from
// SNR; returns the absolute value of the change in SNR
double doppler_snr(double frequency,
    ...
    // double subcarrier_spacing = 10937.5; // FIXME: get from capacity
    // structure
...
double environment_fading(struct environment_class *environment,
    ...
    double
        sensitivity_delta; // change in sensitivity between AWGN and Rayleigh
    ...
    // printf
    //  ("ber=%f fer=%f Pr_threshold_ber=%f frame_size=%d gamma=%f
    //  rx_sensitivity=%f Pr=%f   fer2=%f\n",
    //    ber, fer, Pr_threshold_ber, frame_size, gamma, rx_sensitivity, Pr,
    //    fer2);
