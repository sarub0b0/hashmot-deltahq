...
// return SUCCESS on succes, ERROR on error
int active_tag_compute_interference(struct connection_class *connection,
    ...
    active_tag_loss_rate(
        &virtual_connection, scenario, &(virtual_connection.loss_rate));
    ...
    connection->interference_fer +=
        (1.0 / (9 * 9) * (1 - virtual_connection.frame_error_rate));
    ...
    for (connection_i = 0; connection_i < scenario->connection_number;
        ...
        if (connection != &(scenario->connections[connection_i])) {
            ...
            if ((connection->standard == ACTIVE_TAG) &&
                ...
                DEBUG("--------------------------------------------");
                DEBUG(
                    "Interference between active tags '%s' and '%s' detected \
=> determine effects",
