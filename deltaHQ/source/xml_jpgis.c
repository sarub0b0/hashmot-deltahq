...
static void XMLCALL xml_jpgis_start_element(void *user_data, const char *el,
                                            const char **attr) {
          ...
          if (xml_jpgis->objects[j].load_from_jpgis_file == TRUE)
            ...
            if (strcmp(xml_jpgis->objects[j].name, attr[i + 1]) == 0) {
              ...
              if (((strcmp(el, BUILDING_EDGE_LABEL) == 0) &&
                ...
                INFO("\tObject to be loaded='%s'", xml_jpgis->objects[j].name);
...
static void XMLCALL xml_jpgis_end_element(void *user_data, const char *el) {
  ...
  // helping pointer
  struct object_class *crt_object = &(xml_jpgis->objects[xml_jpgis->object_j]);
  ...
  if ((strcmp(el, BUILDING_EDGE_LABEL) == 0) ||
    ...
    if (xml_jpgis->object_found == TRUE) {
      ...
      if (xml_jpgis->object_j == -1) {
        DEBUG("Check whether the object is in any of the defined regions, and "
              "add it to scenario object array if this is the case.");
#ifdef MESSAGE_DEBUG
        object_print(&(xml_jpgis->temp_object));
#endif
        ...
        for (j = 0; j < xml_jpgis->object_number; j++) {
          ...
          if ((strcmp(el, BUILDING_EDGE_LABEL) == 0) &&
              (xml_jpgis->objects[j].type == ROAD_OBJECT)) {
            DEBUG("Ignoring 'building' object for 'road' region '%s'.",
                  xml_jpgis->objects[j].name);
            continue;
          }
          ...
          if ((strcmp(el, ROAD_EDGE_LABEL) == 0) &&
              (xml_jpgis->objects[j].type == BUILDING_OBJECT)) {
            DEBUG("Ignoring 'road' object for 'building' region '%s'.",
                  xml_jpgis->objects[j].name);
            continue;
          }

          DEBUG("Current element is '%s', region is type '%d'", el,
                xml_jpgis->objects[j].type);
          ...
          DEBUG("Check if object '%s' is in region '%s' with type=%d [(%f,%f); "
                "(%f,%f)]",
          ...
          for (k = 0; k < xml_jpgis->temp_object.vertex_number; k++) {
            if (point_in_object(xml_jpgis->temp_object.vertices[k].c[0],
                                xml_jpgis->temp_object.vertices[k].c[1], rx1,
                                ry1, rx2, ry2)) {
          ...
          if (vertex_match == xml_jpgis->temp_object.vertex_number) {
            DEBUG("All vertices of object '%s' are in region '%s' => adding to "
                  "scenario",
                  xml_jpgis->temp_object.name, xml_jpgis->objects[j].name);
            strncpy(xml_jpgis->temp_object.environment,
                    xml_jpgis->objects[j].environment, MAX_STRING - 1);
            // copy height information
            xml_jpgis->temp_object.height = xml_jpgis->objects[j].height;

            if (scenario_add_object(xml_jpgis->scenario,
...
longitude=%.9f",
            xml_jpgis->latitude, xml_jpgis->longitude);
      ...
      // copy converted data
      (target_object->vertices[xml_jpgis->coordinate_i]).c[0] = point_xyz.c[0];
      (target_object->vertices[xml_jpgis->coordinate_i]).c[1] = point_xyz.c[1];

      // FIXME: set z coordinate to a good value (use altitude
      // value from JPGIS file for point_blh.c[2] above?!)
      (target_object->vertices[xml_jpgis->coordinate_i]).c[2] = point_xyz.c[2];
...
void XMLCALL xml_jpgis_element_text(void *user_data, const char *xmlData,
                                    int length) {
  // get a pointer to the user data in our XML object
  struct xml_jpgis_class *xml_jpgis = (struct xml_jpgis_class *)user_data;
...
longitude=%.9f", xml_jpgis->latitude, xml_jpgis->longitude);
        ...
        {
          WARNING("Maximum number of coordinates (%d) reached when loading
object", MAX_VERTICES);
          xml_jpgis->error = TRUE;
        }
    }
