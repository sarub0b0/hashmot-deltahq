...
void *wimax_get_interface_adapter(struct connection_class *connection,
...
int wimax_init_ns3_adapter(struct parameters_802_16 *adapter) {
    // capacity variable
    ...
    if (capacity_update_all(
            &capacity, WIMAX_DEFAULT_SYS_BW, mcs, MIMO_TYPE_SISO, 1, 1) ==
        ERROR) {
        WARNING(
            "Error updating system bandwidth and MCS for the \
capacity calculation structure.\n");
        return ERROR;
    }

    // printf("WiMAX values: bandwidth = %.2f MCS = %2d: PHY data rate [Mbps]:
    // DL
    // = %5.2f UL = %5.2f\n", WIMAX_DEFAULT_SYS_BW, mcs, capacity.dl_data_rate,
    ...
    for (i = 0; i < WIMAX_RATES_NUMBER; i++) {
        ...
        if (mcs < QAM64_56) {
            ...
            if (capacity_update_mcs(&capacity, mcs) == ERROR) {
                printf(
                    "Error updating MCS for the capacity calculation \
structure.\n");
                return ERROR;
            }
            //	  printf("WiMAX values: bandwidth = %.2f MCS = %2d: PHY data
            // rate [Mbps]: DL = %5.2f UL = %5.2f\n", WIMAX_DEFAULT_SYS_BW, mcs,
            // capacity.dl_data_rate, capacity.ul_data_rate);
        }
    }
...
double wimax_min_threshold(struct capacity_class *capacity) {
    ...
    // should be around -76 dB (also given in text) and the equation gives
    // -76.485
...
int wimax_fer(struct connection_class *connection,
    ...
    // FIXME: check
    // int OFDM_Overhead = 22;     // OFDM ovearhed in bits (service field +
    // tail
    // bits)
    ...
    void *adapter = wimax_get_interface_adapter(
        connection,
    ...
    if (environment->num_segments > 0) {
        ...
        // interference noise
        combined_noise =
            add_powers(environment->noise_power[environment->num_segments - 1],
    ...
    struct capacity_class *capacity =
        &(((scenario->nodes[connection->from_node_index])
               .interfaces[connection->from_interface_index])
              .wimax_params);
    capacity->Nt = ((scenario->nodes[connection->from_node_index])
                        .interfaces[connection->from_interface_index])
                       .antenna_count;
    capacity->Nr = ((scenario->nodes[connection->to_node_index])
                        .interfaces[connection->to_interface_index])
                       .antenna_count;
    ...
    fer_value =
        environment_fading(environment,
                           adapter_802_16->Pr_threshold_ber,
                           connection->packet_size + header,
                           adapter_802_16->Pr_thresholds[operating_rate],
                           connection->SNR + WIMAX_STANDARD_NOISE);

    // DEBUG(
    //    "Pr_threshold_fer=%f model_alpha=%f Pr_thresholds[%d]=%f power=%f "
    //    "(SNR=%f) => fer=%f",
    //    adapter_802_16->Pr_threshold_fer,
    //    adapter_802_16->model_alpha,
    //    operating_rate,
    //    adapter_802_16->Pr_thresholds[operating_rate],
    //    connection->SNR + WIMAX_STANDARD_NOISE,
    //    connection->SNR,
    ...
    // printf("Pr=%f operating_rate=%d mcs=%d Pr_thresholds[operating_rate]=%f
    // fer=%f\n", connection->Pr, operating_rate, capacity->mcs,
    // adapter_802_16->Pr_thresholds[operating_rate], fer_value);
...
int wimax_delay_jitter(struct connection_class *connection,
    ...
    (*delay) =
        node_rx->internal_delay + node_tx->internal_delay + (*variable_delay);
...
int wimax_bandwidth(struct connection_class *connection,
    ...
    /*
    printf("bandwidth = %.2f MCS = %2d: PHY data rate [Mbps]: DL = %5.2f UL =
    %5.2f\n", capacity->system_bandwidth, capacity->mcs, capacity->dl_data_rate,
    capacity->ul_data_rate);
    */
    return SUCCESS;
}
...
int wimax_connection_update(struct connection_class *connection,
    ...
    // printf("tx.mcs=%d rx.mcs=%d =>
    // operating_rate=%d\n",node_tx->interfaces[0].wimax_params.mcs,
    // node_rx->interfaces[0].wimax_params.mcs,connection->operating_rate );
...
int capacity_update_all(struct capacity_class *capacity,
    ...
    if (capacity->system_bandwidth == SYS_BW_1) {
        ...
        capacity->dl_signaling_overhead =
            DL_PREAMBLE_OVERHEAD +
            DL_ADDITIONAL_OVERHEAD_16; // FIXME: guessed from [2] Table 2
        ...
        capacity->dl_signaling_overhead =
            DL_PREAMBLE_OVERHEAD + DL_ADDITIONAL_OVERHEAD_16; // [2] Table 2
        capacity->ttg_rtg_duration = 1.61;
        ...
        capacity->dl_signaling_overhead =
            DL_PREAMBLE_OVERHEAD + DL_ADDITIONAL_OVERHEAD_12; // [2] Table 2
        capacity->ttg_rtg_duration = 1.61;
        ...
        capacity->dl_signaling_overhead =
            DL_PREAMBLE_OVERHEAD +
            DL_ADDITIONAL_OVERHEAD_12; // FIXME: guessed from [2] Table 2
        capacity->ttg_rtg_duration = 1.61;
    ...
    capacity->OFDMA_symbol_time =
        capacity->data_symbol_time + capacity->guard_time;
    ...
    if (1) {
        capacity->dl_symbols =
            (int) floor(capacity->total_symbols * DL_SYMBOL_RATIO);
        capacity->ul_symbols =
            (int) floor(capacity->total_symbols * (1 - DL_SYMBOL_RATIO));
        // printf("Update DL and UL symbol count: dl_symbols=%d ul_symbols=%d
        // (DL_SYMBOL_RATIO=%.2f)\n", capacity->dl_symbols,
        // capacity->ul_symbols,
        // DL_SYMBOL_RATIO);
    }
    ...
    if (capacity->system_bandwidth == SYS_BW_1) {
        ...
        // applied empirical formula given that FFT=1024 => same with 10 MHz
        // case;
        // results validated using [2] Table 5
    ...
    capacity->dl_used_subcarriers =
        capacity->fft_size - capacity->dl_guard_subcarriers;
    ...
    // * on average there are 4 pilots in a subchannel (8 pilots / 2 symbol
    // time)
    ...
    capacity->ul_used_subcarriers =
        capacity->fft_size - capacity->ul_guard_subcarriers;

    // formula deduced based on [1] Sec. 2, paragraph 6, and verified using
    // the values in Table 2; the following were considered:
    // * 1 DC subcarrier is sent without power;
    // * there are 24 subcarriers in a subchannel
    // * on average there are 8 pilots in a subchannel (24 pilots / 3 symbol
    // time)
...
int capacity_update_mcs(struct capacity_class *capacity, int mcs) {
    ...
    if (capacity->dl_bytes_per_slot == -1) return ERROR;
    ...
    if (capacity->ul_bytes_per_slot == -1) return ERROR;
...
void capacity_print(struct capacity_class *capacity) {
    ...
    printf("Total data rate = %.2f Mbps\n",
           (capacity->dl_data_rate + capacity->ul_data_rate) / 1e6);
    printf("MIMO type: %d\n", capacity->mimo_type);
...
double capacity_bytes_per_slot(struct capacity_class *capacity, int downlink) {
    ...
    if (capacity->mcs != QPSK_18) bytes_per_symbol = round(bytes_per_symbol);
