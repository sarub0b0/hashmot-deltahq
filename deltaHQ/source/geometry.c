...
void ll2en(struct coordinate_class *point_blh,
  ...
  double VI = (nu / 120) * pow(cos(phi_rad), 5) *
              (5 - (18 * pow(tan(phi_rad), 2)) + pow(tan(phi_rad), 4) +
               (14 * eta_squared) - (58 * pow(tan(phi_rad), 2) * eta_squared));

  DEBUG("phi=%.8e lambda=%.8e phi_rad=%.8e lambda_rad=%.8e phi0_rad=%.8e \
lambda0_rad=%.8e aF0=%.8e bF0=%.8e",
        phi, lambda, phi_rad, lambda_rad, phi0_rad, lambda0_rad, aF0, bF0);

  DEBUG("e2=%.8e n=%.8e nu=%.8e rho=%.8e eta2=%.8e dl=%.8e M=%.8e I=%.8e \
II=%.8e III=%.8e IIIA=%.8e IV=%.8e V=%.8e VI=%.8e",
        e_squared, n, nu, rho, eta_squared, delta_lambda_rad, M, I, II, III,
        IIIA, IV, V, VI);
  ...
  point_xyz->c[1] = I + (II * pow(delta_lambda_rad, 2)) +
                    (III * pow(delta_lambda_rad, 4)) +
                    (IIIA * pow(delta_lambda_rad, 6));
...
void en2ll(struct coordinate_class *point_xyz,
  ...
  double XIIA = (pow(cos(phi_d), -1) / (5040 * pow(nu, 7))) *
                (61 + (662 * (pow(tan(phi_d), 2))) +
                 (1320 * (pow(tan(phi_d), 4))) + (720 * (pow(tan(phi_d), 6))));

  // latitude
  point_blh->c[0] = (180 / M_PI) * (phi_d - (pow(Et, 2) * VII) +
                                    (pow(Et, 4) * VIII) - (pow(Et, 6) * IX));

  // longitude
  point_blh->c[1] = (180 / M_PI) * (lambda0_rad + (Et * X) - (pow(Et, 3) * XI) +
                                    (pow(Et, 5) * XII) - (pow(Et, 7) * XIIA));
