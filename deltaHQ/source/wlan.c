...
struct parameters_802_11g cisco_aironet_abg_bg = {
    ...
    {-94,
     -93,
     -92,
     -91.33 /* modified from -86 */,
     -90.67 /* modified from -86 */,
     -90,
     -88 /* modified from -86 */,
     -86,
     -84,
     -80,
     -75,
     -71}, // Pr_thresholds (from low to high)
    0.08,  // Pr_threshold_fer
...
struct parameters_802_11a cisco_aironet_abg_a = {
    ...
    {-87,
     -85.75,
     -84.5,
     -83.25 /* last 3 values modified from -87 */,
     -82,
     -79,
     -74,
     -72},
...
double g_operating_rates[] = {
    1e6, 2e6, 5.5e6, 6e6, 9e6, 11e6, 12e6, 18e6, 24e6, 36e6, 48e6, 54e6};
...
// update the changing properties of a connection (e.g., relative distance,
// Pr) after the state of the system changed; return SUCCESS on succes, ERROR
// on error
    ...
    if (environment->num_segments > 0) {
        ...
        if (environment->num_segments == 1) {
            ...
            double antenna_dir_attenuation_tx =
                antenna_directional_attenuation(
                    node_tx,
            ...
            if (connection->standard == WLAN_802_11A)
                if (node_tx->interfaces[connection->from_interface_index]
                        .Pt >=
                    ...
                    connection->Pr =
                        node_rx->interfaces[connection->to_interface_index]
                            .Pr0_a;
                    printf(
                        "deltaQ: DEBUG: Interop2014: Using Pt of interface "
                        "%d "
                        "instead of that of %d because it is larger!\n",
                        node_rx->interfaces[connection->to_interface_index]
                            .id,
                        node_tx->interfaces[connection->from_interface_index]
                            .id);
        ...
        } else // dynamic environment
        {
            ...
            for (i = 1; i < environment->num_segments; i++) {
                ...
                connection->Pr -=
                    (10 * environment->alpha[i] *
                     (log10(current_distance + environment->length[i]) -
...
void *wlan_get_interface_adapter(struct connection_class *connection,
    ...
    if (connection->standard == WLAN_802_11B) {
        if (interface->adapter_type == ORINOCO)
            ...
            WARNING(
                "Defined WLAN adapter type %d is incompatible with \
...
int compute_channel_interference(struct connection_class *connection,
    ...
    if (scenario->nodes[connection_i->from_node_index]
    ...
    environment_print(
        &(scenario
              ->environments[virtual_connection.through_environment_index]));
#endif
    ...
    channel_distance = (int) abs(connection->channel - connection_i->channel);
    ...
    if (connection_i->standard == WLAN_802_11B ||
        (connection_i->standard == WLAN_802_11G &&
         (connection_i->operating_rate == 0 ||
          connection_i->operating_rate == 1 ||
          connection_i->operating_rate == 2 ||
          connection_i->operating_rate == 5))) {
        INFO(
            "Interfering connection is 'b'/'g' => DSSS/CCK attenuation "
            "model");
        if (channel_distance == 0)
            ...
            attenuation =
                10 * log10((44.0 - channel_distance * 5) / 44.0) - 30;
        /*
    ...
    } else // OFDM encoding
    {
        ...
        if (channel_distance == 0)
            ...
            attenuation =
                10 * log10((40.0 - channel_distance * 5) / 40.0) - 28;
        /*
           else
    ...
    // then this power will indeed have effects of noise and induce frame
    // errors
    else if ((connection->standard == WLAN_802_11B &&
              virtual_connection.Pr < ((struct parameters_802_11b *) adapter)
                                          ->Pr_thresholds[B_RATE_1MBPS]) ||
             (connection->standard == WLAN_802_11G &&
              virtual_connection.Pr < ((struct parameters_802_11g *) adapter)
                                          ->Pr_thresholds[G_RATE_1MBPS]) ||
             (connection->standard == WLAN_802_11A &&
              virtual_connection.Pr < ((struct parameters_802_11a *) adapter)
...
int wlan_interference(struct connection_class *connection,
    ...
    for (connection_i = 0; connection_i < scenario->connection_number;
        ...
        if (connection != &(scenario->connections[connection_i])) {
            ...
            // check if connections are both either 'b'/'g' type or both 'a'
            // type
            if (((connection->standard == WLAN_802_11B ||
                  connection->standard == WLAN_802_11G) &&
                ...
                if (connection->standard == WLAN_802_11B ||
                    connection->standard == WLAN_802_11G)
                    INFO(
                        "Interference 'b'/'g' <-> 'b'/'g' detected => \
determine effects");
                else // interference a-a
                    INFO(
                        "Interference 'a' <-> 'a' detected => determine "
                        "effects");
...
int wlan_fer(struct connection_class *connection,
    ...
    int OFDM_Overhead =
        22; // OFDM ovearhed in bits (service field + tail bits)
    ...
    if (connection->standard == WLAN_802_11B) {
        ...
        // INCORRECT
        fer1 =
            1 - pow(1 - fer1, (connection->packet_size + header) / PSDU_DSSS);

        // limit error rate for numerical reasons
        if (fer1 > MAXIMUM_ERROR_RATE) fer1 = MAXIMUM_ERROR_RATE;

        // use model 2 (SNR based)
        // BER fitted using a*e^(b*x)
        if (environment->num_segments > 0) {
            // use direct formula for the case of 1 segment
            if (environment->num_segments == 1) INFO("use wlan_add_powers");
            connection->SNR = connection->Pr -
                ...
                /*
                   ...
                   // if all power sources had minimum level, just substract
                   it,
                   ...
                   connection->SNR = connection->Pr - 10 *
                   log10(noise_power_sum);
                 */
            }
        } else {
        ...
        if (adapter_802_11b->use_model2 == TRUE)
            ber2 = adapter_802_11b->model2_a[operating_rate] *
                   exp(adapter_802_11b->model2_b[operating_rate] *
                       connection->SNR);
        else
            ber2 = 0;

        // limit error rate for numerical reasons
        if (ber2 > MAXIMUM_ERROR_RATE) ber2 = MAXIMUM_ERROR_RATE;

        // compute fer2 from ber2 and packet_size
        fer2 = 1 - pow((1 - ber2), 8 * connection->packet_size);

        // limit error rate for numerical reasons
        if (fer2 > MAXIMUM_ERROR_RATE) fer2 = MAXIMUM_ERROR_RATE;
        ...
        if (environment->num_segments > 0) {
            ...
            // interference noise
            combined_noise = add_powers(
                environment->noise_power[environment->num_segments - 1],
                connection->interference_noise,
                MINIMUM_NOISE_POWER);
            ...
            if (operating_rate == 0 || operating_rate == 1 ||
                ...
                fer1 = adapter_802_11g->Pr_threshold_fer *
                       exp(adapter_802_11g->model1_alpha *
                           (adapter_802_11g->Pr_thresholds[operating_rate] -
                            connection->Pr));

                // this FER corresponds to the standard value PSDU_DSSS,
                // therefore needs to be adjusted for different packet sizes
                // fer1 = (fer1 * (connection->packet_size + header)) /
                // PSDU_DSSS; INCORRECT
                fer1 =
                    1 - pow(1 - fer1,
                            (connection->packet_size + header) / PSDU_DSSS);
        ...
        if (adapter_802_11g->use_model1 == TRUE)
            if (operating_rate == 0 || operating_rate == 1 ||
                ...
                double relative_velocity = motion_relative_velocity(
                    scenario,
                    &(scenario->nodes[connection->from_node_index]),
                    &(scenario->nodes[connection->to_node_index]));
                double doppler_snr_value =
                    doppler_snr(FREQUENCY_BG,
                                WIFI_SUBCARRIER_SPACING * 1e3,
                                relative_velocity,
                                connection->SNR);
                /*
                   printf("freq=%.2f GHz  subcarrier_spacing=%.1f Hz  \
                   rel_velocity=%f  SNR=%f  ", FREQUENCY_BG/1e9,
                   WIFI_SUBCARRIER_SPACING*1e3,
        ...
        if (adapter_802_11a->use_model1 == TRUE) {
            fer1 = adapter_802_11a->Pr_threshold_per *
                   exp(adapter_802_11a->model1_alpha *
                       (adapter_802_11a->Pr_thresholds[operating_rate] -
                        connection->Pr));
        ...
        if (adapter_802_11a->use_model1 == TRUE) {
            ...
            //
            // TODO doppler_snr
            // get velocity from json
            double relative_velocity = motion_relative_velocity(
                scenario,
                &(scenario->nodes[connection->from_node_index]),
                &(scenario->nodes[connection->to_node_index]));
            double doppler_snr_value =
                doppler_snr(FREQUENCY_A,
                            WIFI_SUBCARRIER_SPACING * 1e3,
                            relative_velocity,
                            connection->SNR);
            /*
               printf("freq=%.2f GHz  subcarrier_spacing=%.1f Hz  \
               rel_velocity=%f  SNR=%f  ", FREQUENCY_A/1e9,
               WIFI_SUBCARRIER_SPACING*1e3,
               relative_velocity, connection->SNR);
               printf("SNR decrease due to Doppler shift = %.3f dB\n",
               doppler_snr_value);
    ...
    // limit error rate for numerical reasons
    if ((*fer) > MAXIMUM_ERROR_RATE) (*fer) = MAXIMUM_ERROR_RATE;
...
// already calculated);
// return SUCCESS on succes, ERROR on error
int wlan_do_compute_loss_rate(struct connection_class *connection,
                              double *loss_rate) {
...
int wlan_operating_rate(struct connection_class *connection,
    ...
    // rate changes downwards if the probability of losing two consecutive
    // frames
    ...
    else if (pow(1 - connection->frame_error_rate, 10) >
             ARF_FER_UP_THRESHOLD) {
        if ((connection->standard == WLAN_802_11B) ||
            (connection->standard == WLAN_802_11G) ||
            (connection->standard == WLAN_802_11A)) {
...
int wlan_ppdu_duration(struct connection_class *connection,
    ...
    if (connection->standard == WLAN_802_11A) {
        ...
        ACK_symbols = (int) ceil((double) (OFDM_Overhead + ACK_Size) /
                                 a_OFDM_data[connection->operating_rate]);

        (*ppdu_duration) = DIFS + PHY_Overhead +
                           data_symbols * Symbol_Duration + SIFS +
                           PHY_Overhead + ACK_symbols * Symbol_Duration;
        ...
        if (connection->packet_size > connection->RTS_CTS_threshold)
            (*ppdu_duration) +=
                ((RTS_Size + PHY_Overhead) * 1e6 / basic_rate_a +
        ...
        (*ppdu_duration) =
            DIFS + PHY_Overhead +
            ceil((MAC_Overhead + Frame_Body) * 1e6 /
        ...
        DEBUG(
            "DIFS=%d PHY_Overhead=%d PHY_frame=%f SIFS=%d PHY_Overhead=%d "
            "ACK_frame=%f",
            DIFS,
            PHY_Overhead,
            ceil((MAC_Overhead + Frame_Body) * 1e6 /
        ...
        if (connection->packet_size > connection->RTS_CTS_threshold)
            (*ppdu_duration) +=
                ((RTS_Size + PHY_Overhead) * 1e6 / basic_rate_bg +
        ...
        if (connection->operating_rate == 0 ||
            ...
            (*ppdu_duration) =
                DIFS + PHY_Overhead +
                ceil((MAC_Overhead + Frame_Body) * 1e6 /
                     g_operating_rates[connection->operating_rate]) +
                SIFS
                // apparently ACK is sent at the same rate with data!!!!!
                + PHY_Overhead
                //+ ceil((PHY_Overhead + ACK_Size)*1e6/basic_rate_bg);
        ...
        } else // ERP-OFDM rates: 6, 9, 12, 18, 24, 36, 48, 54 Mb/s
        {
            ...
            data_symbols = (int) ceil(
                (double) (OFDM_Overhead + MAC_Overhead + Frame_Body) /
                g_OFDM_data[connection->operating_rate]);
            ACK_symbols = (int) ceil((double) (OFDM_Overhead + ACK_Size) /
                                     g_OFDM_data[connection->operating_rate]);

            (*ppdu_duration) =
                DIFS + PHY_Overhead + data_symbols * Symbol_Duration +
                Signal_Extension + SIFS + PHY_Overhead +
        ...
        // in compatibility mode, we need to send CTS-to-self (CTS + SIFS)
            ...
            (*ppdu_duration) +=
                ((CTS_Size + PHY_Overhead) * 1e6 / basic_rate_bg + SIFS);
    } else {
...
int wlan_delay_jitter(struct connection_class *connection,
    ...
    if (connection->delay_defined == FALSE) {
        ...
        if (connection->consider_interference == TRUE &&
            ...
            DEBUG(
                "Delay interference for node with id=%d (\
concurrent_stations=%d)",
                node_rx->id,
    ...
    if (connection->jitter_defined == FALSE) {
        ...
        if (connection->consider_interference == TRUE &&
            ...
            DEBUG(
                "Jitter interference for node with id=%d (\
concurrent_stations=%d)",
                node_rx->id,
                (int) (connection->concurrent_stations));
        }
...
// uses the field "frame_error_rate" internally;
// return SUCCESS on succes, ERROR on error
int wlan_do_compute_delay_jitter(struct connection_class *connection,
                                 double *avg_delay,
                                 double *avg_jitter,
    ...
    // values for delay and jitter corresponding to 'i' retransmissions (in
    // us) NOTE: the jitter values depend on the average delay, therefore they
    // change
    //       with FER
    ...
    if (connection->standard == WLAN_802_11B ||
        (connection->standard == WLAN_802_11G &&
         connection->compatibility_mode == TRUE)) {
        for (i = 0; i < MAX_TRANSMISSIONS; i++) CW[i] = CW_b[i];
    }
    ...
    // use a/g-type congestion window
        ...
        for (i = 0; i < MAX_TRANSMISSIONS; i++) CW[i] = CW_ag[i];
    } else {
        WARNING("Connection standard '%d' undefined", connection->standard);
        return ERROR;
    }
    ...
    D_avg = D[0];
    for (i = 1; i <= r; i++) D_avg += D[i] * pow(FER, i);
...
int wlan_operating_rate_index(struct connection_class *connection,
    ...
    if (connection->standard == WLAN_802_11A) {
        for (i = A_RATE_6MBPS; i <= A_RATE_54MBPS; i++)
            if (a_operating_rates[i] == op_rate) return i;
    } else if (connection->standard == WLAN_802_11B) {
        for (i = B_RATE_1MBPS; i <= B_RATE_11MBPS; i++)
            if (b_operating_rates[i] == op_rate) return i;
    } else if (connection->standard == WLAN_802_11G) {
        for (i = G_RATE_1MBPS; i <= G_RATE_54MBPS; i++)
            if (g_operating_rates[i] == op_rate) return i;
    } else
