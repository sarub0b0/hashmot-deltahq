...
void coordinate_init(struct coordinate_class *coordinate,
                     const char *name,
                     double c0,
                     double c1,
                     double c2) {
...
void coordinate_copy(struct coordinate_class *coord_dest,
    ...
    for (i = 0; i < MAX_COORDINATES; i++) coord_dest->c[i] = coord_src->c[i];
}
...
void coordinate_vector_difference_2D(struct coordinate_class *result,
    ...
    for (i           = 0; i < MAX_COORDINATES_2D; i++)
        result->c[i] = vector1->c[i] - vector2->c[i];
    for (i = MAX_COORDINATES_2D; i < MAX_COORDINATES; i++) result->c[i] = 0;
}
...
void coordinate_multiply_scalar(struct coordinate_class *result,
    ...
    for (i = 0; i < MAX_COORDINATES; i++) result->c[i] = vector->c[i] * scalar;
}
...
int coordinate_distance_to_segment(const struct coordinate_class *point,
    ...
    for (i = 0; i < MAX_COORDINATES; i++)
        intersection->c[i] =
            segment_start->c[i] + u * (segment_end->c[i] - segment_start->c[i]);
...
// "Minimum Distance Between a Point and a Line" by P. Bourke (1988)
// http://local.wasp.uwa.edu.au/~pbourke/geometry/pointline/
int coordinate_distance_to_segment_2D(
    const struct coordinate_class *point,
    const struct coordinate_class *segment_start,
    const struct coordinate_class *segment_end,
    double *distance,
    struct coordinate_class *intersection) {
...
double coordinate_vector_angle_2D(const struct coordinate_class *vector) {
    ...
    for (i = 0; i < MAX_COORDINATES_2D; i++) sum += vector->c[i] * vector->c[i];

    if (sum < EPSILON)
        angle = 0;
    else {
        angle                       = acos(vector->c[0] / sqrt(sum));
        if (vector->c[1] < 0) angle = 2 * M_PI - angle;
    }
