...
void object_init(struct object_class *object,
                 const char *name,
    ...
    // if set to FALSE, the coordinates are given as
    // latitude-longitude-altitude and need to be converted before further
    // processing object->is_metric = TRUE;
...
void object_init_rectangle(struct object_class *object,
                           const char *name,
                           const char *environment,
                           double x1,
                           double y1,
...
int object_init_index(struct object_class *object,
    ...
    if (object->environment_index == INVALID_INDEX)
        ...
        for (i = 0; i < scenario->environment_number; i++)
            if (strcmp(scenario->environments[i].name, object->environment) ==
                0) {
    ...
    if (object->environment_index == INVALID_INDEX) {
        WARNING(
            "Object attribute 'environment' with value '%s' doesn't exist \
in scenario",
            object->environment);
        return ERROR;
    }
...
// in the (x,y,z) format
void object_print(struct object_class *object) {
    int i;

    printf(
        "  Object '%s': type='%s' environment='%s' vertex_number=%d \
make_polygon=%s height=%.2f load_from_jpgis_file=%s coordinates:\n",
        object->name,
        object_types[object->type],
        object->environment,
        object->vertex_number,
        (object->make_polygon == TRUE) ? "TRUE" : "FALSE",
        object->height,
    ...
    if (object->vertex_number == 0)
        ...
        for (i = 0; i < object->vertex_number; i++)
            printf("\tV[%d](x,y,z)=(%.9f,%.9f,%.9f)\n",
                   i,
                   object->vertices[i].c[0],
                   object->vertices[i].c[1],
...
void object_copy(struct object_class *object_dest,
    ...
    strncpy(
        object_dest->environment, object_src->environment, MAX_STRING - 1);
    ...
    for (i = 0; i < object_dest->vertex_number; i++)
        coordinate_copy(&(object_dest->vertices[i]),
                        &(object_src->vertices[i]));
...
int object_check_valid(struct object_class *objects,
                       int object_number,
    ...
    for (i = 0; i < object_number; i++) {
        if (strcmp(objects[i].name, new_object->name) == 0) {
            WARNING("Object with name '%s' already defined",
                    new_object->name);
            return FALSE;
        }
    }
    ...
    if (new_object->load_from_jpgis_file == TRUE &&
        ...
        WARNING(
            "Object '%s' is supposed to be loaded from a JPGIS file, \
but no filename was provided in the QOMET scenario",
            new_object->name);
        return FALSE;
    }
...
int object_intersect(struct coordinate_class *start,
    ...
    for (vertex_i = 0; vertex_i < last_vertex; vertex_i++) {
        ...
        if (segment_intersect(start->c[0],
                              start->c[1],
                              end->c[0],
                              end->c[1],
            ...
            if (include_vertices == FALSE &&
                 ...
                 */
                DEBUG(
                    "Intersection ignored because is vertex, and parameter \
