...
int connection_init(struct connection_class *connection,
    ...
    strncpy(
        connection->through_environment, through_environment, MAX_STRING - 1);
...
int connection_init_standard(struct connection_class *connection,
    ...
    if (standard == WLAN_802_11B) {
        ...
        // we assume now that WLAN adapters start from the lowest rate and go
        // up
        ...
        connection->bandwidth =
            eth_operating_rates[connection->operating_rate];
...
int connection_init_indexes(struct connection_class *connection,
        ...
        for (i = 0; i < scenario->node_number; ++i) {
            ...
            if (strcmp(node->name, connection->from_node) == 0) {
                ...
                } else // try to find from_interface among those of the node
                {
                    ...
                    // if j equals node->interface_number, then the
                    // from_interface
                    // could not be found => return ERROR
                    if (j >= node->interface_number) {
                        WARNING(
                            "Connection attribute '%s' with value '%s' does "
                            "not exist for node '%s'.",
            ...
            if (strcmp(node->name, connection->to_node) == 0) {
                ...
                } else // try to find to_interface among those of the node
                {
                    ...
                    // if j equals node->interface_number, then the
                    // to_interface could not be found => return ERROR
                    if (j >= node->interface_number) {
                        WARNING(
                            "Connection attribute '%s' with value '%s' does "
                            "not exist for node '%s'.",
    ...
    if (connection->from_node_index == INVALID_INDEX) {
        WARNING(
            "Connection attribute 'from_node' with value '%s' doesn't exist \
in scenario",
            connection->from_node);
        return ERROR;
    }
    ...
    if (connection->to_node_index == INVALID_INDEX) {
        WARNING(
            "Connection attribute 'to_node' with value '%s' doesn't exist \
in scenario",
            connection->to_node);
        return ERROR;
    }
    ...
    if (connection->through_environment_index == INVALID_INDEX) {
        WARNING(
            "Connection attribute 'through_environment' with value '%s' \
...
void connection_print(struct connection_class *connection) {
    printf(
        "  Connection: from_node='%s' from_node_index='%d' from_interface=%d "
        "to_node='%s' to_node_index='%d' "
        "to_interface = % d through_environment = '%s'\n ",
        connection->from_node,
        connection->from_node_index,
        connection->from_interface_index,
        connection->to_node,
        connection->to_node_index,
        connection->to_interface_index,
        connection->through_environment);
    ...
    if ((connection->standard == WLAN_802_11B) ||
        ...
        printf(
            "    Properties: packet_size=%d standard='%s' channel=%d "
        ...
        printf(
            "    DeltaQ: FER=%.3f num_retr=%.2f op_rate=%.1fMb/s bw=%.1fMb/s \
loss=%.3f delay=%.2fms jitter=%.2fms\n",
            connection->frame_error_rate,
            connection->num_retransmissions,
            connection_get_operating_rate(connection) / 1e6,
            connection->bandwidth / 1e6,
        ...
        printf(
            "    DeltaQ: FER=%.3f op_rate=%.1fMb/s bw=%.1fMb/s \
loss=%.3f delay=%.2fms jitter=%.2fms\n",
            connection->frame_error_rate,
            connection_get_operating_rate(connection) / 1e6,
            connection->bandwidth / 1e6,
        ...
        printf(
            "    Properties: packet_size=%d standard='%s' \
...
SNR=%.2f consider_interference=%s\n",
        ...
        printf(
            "    DeltaQ: FER=%.3f interference_fer=%.3f num_retr=%.2f \
...
void connection_copy(struct connection_class *connection_dst,
                     struct connection_class *connection_src) {
    int i;

    strncpy(
        connection_dst->from_node, connection_src->from_node, MAX_STRING - 1);
    connection_dst->from_node_index = connection_src->from_node_index;
    strncpy(connection_dst->from_interface,
            connection_src->from_interface,
            MAX_STRING - 1);
    connection_dst->from_interface_index =
        connection_src->from_interface_index;
    ...
    strncpy(connection_dst->to_interface,
            connection_src->to_interface,
            MAX_STRING - 1);
    ...
    strncpy(connection_dst->through_environment,
            connection_src->through_environment,
            MAX_STRING - 1);
...
int connection_do_compute(struct connection_class *connection,
    ...
    if ((connection->standard == WLAN_802_11B) ||
        ...
        if (wlan_retransmissions(
                connection, scenario, &(connection->num_retransmissions)) ==
            ERROR) {
        ...
        if (connection->adaptive_operating_rate == TRUE)
            ...
            if (wlan_operating_rate(connection,
                                    scenario,
        ...
        if (connection->delay_defined == FALSE ||
            ...
            if (wlan_delay_jitter(connection,
                                  scenario,
        ...
        if (connection->bandwidth_defined == FALSE)
            // compute bandwidth based on operating rate, delay and packet
            // size
            if (wlan_bandwidth(connection,
                               scenario,
        ...
        if (connection->loss_rate_defined == FALSE)
            // compute loss rate
            if (ethernet_loss_rate(connection,
                                   scenario,
                                   &(connection->loss_rate)) == ERROR) {
        ...
        if (connection->adaptive_operating_rate == TRUE)
            ...
            if (ethernet_operating_rate(connection,
                                        scenario,
                                        &(connection->new_operating_rate)) ==
                ERROR) {
        ...
        if (connection->delay_defined == FALSE &&
            ...
            if (ethernet_delay_jitter(connection,
                                      scenario,
                                      &(connection->variable_delay),
                                      &(connection->delay),
                                      &(connection->jitter)) == ERROR) {
        ...
        if (connection->bandwidth_defined == FALSE)
            // compute bandwidth based on operating rate, delay and packet
            // size
            if (ethernet_bandwidth(connection,
                                   scenario,
                                   &(connection->bandwidth)) == ERROR) {
        ...
        if (connection->consider_interference == TRUE) {
            ...
            connection->concurrent_stations = 0;
            // connection->interference_noise =
            // MINIMUM_ACTIVE_TAG_NOISE_POWER;
        }

        if (connection->loss_rate_defined == FALSE)
            // compute loss rate
            if (active_tag_loss_rate(connection,
                                     scenario,
                                     &(connection->loss_rate)) == ERROR) {
        ...
        if (connection->delay_defined == FALSE &&
            connection->jitter_defined == FALSE)
            // compute delay & jitter
            if (active_tag_delay_jitter(connection,
                                        scenario,
                                        &(connection->variable_delay),
                                        &(connection->delay),
                                        &(connection->jitter)) == ERROR) {
        ...
        if (connection->bandwidth_defined == FALSE)
            // compute bandwidth based on operating rate, delay and packet
            // size
            if (active_tag_bandwidth(connection,
                                     scenario,
                                     &(connection->bandwidth)) == ERROR) {
        ...
        if (connection->loss_rate_defined == FALSE)
            // compute loss rate
            if (zigbee_loss_rate(connection,
                                 scenario,
                                 &(connection->loss_rate)) == ERROR) {
        ...
        if (connection->delay_defined == FALSE &&
            connection->jitter_defined == FALSE)
            // compute delay & jitter
            if (zigbee_delay_jitter(connection,
                                    scenario,
                                    &(connection->variable_delay),
                                    &(connection->delay),
                                    &(connection->jitter)) == ERROR) {
        ...
        if (connection->bandwidth_defined == FALSE)
            // compute bandwidth based on operating rate, delay and packet
            // size
            if (zigbee_bandwidth(connection,
                                 scenario,
                                 &(connection->bandwidth)) == ERROR) {
        ...
        if (wimax_operating_rate(
                connection, scenario, &(connection->new_operating_rate)) ==
            ERROR) {
        ...
        if (connection->delay_defined == FALSE &&
            connection->jitter_defined == FALSE)
            // compute delay & jitter
            if (wimax_delay_jitter(connection,
                                   scenario,
                                   &(connection->variable_delay),
                                   &(connection->delay),
                                   &(connection->jitter)) == ERROR) {
...
int connection_deltaQ(struct connection_class *connection,
    ...
    if (connection->from_node_index == INVALID_INDEX) {
        WARNING(
            "Connection attribute 'from_node'=%s doesn't exist \
in scenario",
            connection->from_node);
        return ERROR;
    }
    ...
    if (connection->to_node_index == INVALID_INDEX) {
        WARNING(
            "Connection attribute 'to_node'=%s doesn't exist \
in scenario",
            connection->to_node);
        return ERROR;
    }
    ...
    if (connection->through_environment_index == INVALID_INDEX) {
        WARNING(
            "Connection attribute 'through_environment'=%s \
doesn't exist in scenario.\n",
            connection->through_environment);
        return ERROR;
    }
    ...
    if (scenario->environments[connection->through_environment_index]
            .is_dynamic == TRUE) {
    ...
    if (connection_do_compute(connection, scenario, deltaQ_changed) ==
        ERROR) {
    ...
    if (connection->fixed_deltaQ_number < MAX_FIXED_DELTAQ) {
        if ((connection->fixed_deltaQ_number > 0) &&
            (connection->fixed_deltaQs[connection->fixed_deltaQ_number - 1]
                 .end_time > fixed_deltaQ->start_time)) {
            WARNING(
                "New fixed_deltaQ structure has a start time preceding \
...
void *thread_connection_init_index(void *arg) {
    struct thread_connection_class *a =
        (struct thread_connection_class *) arg;
...
void *thread_connection_init_status(void *arg) {
    struct thread_connection_class *a =
        (struct thread_connection_class *) arg;
    struct scenario_class *scenario;
    struct connection_class *connections;
    ...
    DEBUG("conn: start=%d end=%d", start, end);
    ...
    for (int i = start; i < end; ++i) {
        ...
        do {
            ...
            if (num_iterations >= MAXIMUM_PRECOMPUTE) {
                DEBUG(
                    "Maximum number of iterations (%d) was reached during "
                    "precomputation phase without reaching steady state",
                    MAXIMUM_PRECOMPUTE);
                break;
            }
        } while (1);

        INFO("--- Connection %d initializing finished ---", i);
    }
    DEBUG(
        "* Connection validation and initialization done (%d connections)    "
        " "
        "\n",
...
int connection_init_neighbor(struct scenario_class *scenario,
    ...
    if (0 < own_id) {
        ...
        // for (int i = 0; i < scenario->node_number; i++) {
        //     own_id = i;
        //     for (int j = 0; j < scenario->node_number; j++) {
        //         if (own_id == j) {
        //             continue;
        //         }
        //         nb_id = j;
        ...
        //             nb_id--;
        //         }
        //         conn_i         = own_id * (scenario->node_number - 1) +
        //         nb_id; neighbor[ni++] = &scenario->connections[conn_i];
        //         // connection_print(&scenario->connections[conn_i]);
        ...
        //             nb_id--;
        //         }
        //         conn_i         = j * (scenario->node_number - 1) + nb_id;
        //         neighbor[ni++] = &scenario->connections[conn_i];
        //         // connection_print(&scenario->connections[conn_i]);
        //     }
        // }
    ...
    //     for (int node_i = 0; node_i < scenario->node_number; node_i++)
    //     {
    //         struct node_class *node;
    //         node = &scenario->nodes[node_i];
    //         for (int if_i = 0; if_i < node->interface_number; if_i++) {
    //             node->interfaces[if_i].Pt_arrival_distance = 243;
    //         }
    //     }
    //     calc->calculation_number = 0;
    //     for (int conn_i = 0; conn_i < scenario->connection_number;
    //     conn_i++) {
    //         struct connection_class *conn;
    //         struct node_class *rx, *tx;
    //         double distance;

    //         conn = &scenario->connections[conn_i];
    //         rx   = &scenario->nodes[conn->to_node_index];
    //         tx   = &scenario->nodes[conn->from_node_index];

    //         distance = coordinate_distance(&rx->position,
    //         &tx->position);

    //         if (distance < tx->interfaces[conn->from_interface_index]
    //                            .Pt_arrival_distance) {
    //             // scenario->calculations
    //             calc->connection_indexes[calc->calculation_number] =
    //             conn_i; calc->calculation_number++;
    //         }
    //     }
    // }
    return 0;
...
int connection_update_neighbor() {
