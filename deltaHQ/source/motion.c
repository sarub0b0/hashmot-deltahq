...
char *motion_types[] = {
    "linear", "circular", "rotation", "random_walk", "behavioral", "qualnet"};
...
void motion_init(struct motion_class *motion,
                 int id,
                 char *node_name,
                 int type,
                 double speed_x,
    ...
    coordinate_init(
        &(motion->speed), "motion->speed", speed_x, speed_y, speed_z);

    // rotate motion fields
    coordinate_init(
        &(motion->center), "motion->center", center_x, center_y, 0.0);
    ...
    coordinate_init(
        &(motion->destination), "motion->destination", 0.0, 0.0, 0.0);
...
int motion_load_qualnet_mobility(struct motion_class *motion,
        ...
        while (fgets(line, MAX_STRING, mobility_file) != NULL) {
            ...
            if (scanned_values < 5) {
                WARNING("Incorrect data on line %d in mobility file '%s': %s",
                        ...
                        line);
                WARNING(
                    "Expected format is: <NODE_ID> <TIME> (<COORD1>, \
<COORD2>, <COORD3>) [<AZIMUTH> <ELEVATION>]");
                WARNING(
                    "NOTE: Only the <AZIMUTH> and <ELEVATION> parameters \
are optional.");
            ...
            if (scenario->nodes[motion->node_index].id == node_id) {
                ...
                if (i > 0) {
                    ...
                    if (time < motion->trace_records[i - 1].time) {
                        WARNING(
                            "Time %f of new record is inferior to the time "
                            "of "
                            "the last added record (%f). Aborting...",
                            time,
                            motion->trace_records[i - 1].time);
                        fclose(mobility_file);
                        return ERROR;
                    }
                    // if a new record has the same time with the last added
                    // one,
                    // replace the old record with the new one
                    else if (time == motion->trace_records[i - 1].time) {
                        INFO(
                            "Time %f of new record equals the time of the "
                            "last "
                            "added record (%f). Replacing last record with "
                            "the "
                            "new one...",
                            time,
                            motion->trace_records[i - 1].time);
                ...
                if (i < MAX_MOBILITY_RECORDS - 1)
                    ...
                    WARNING(
                        "Maximum number of motion trace records (%d) "
                        "exceeded.",
        ...
        DEBUG(
            "Read %u lines from mobility file. Found %d records for node \
with id '%d'",
            line_nr,
            i,
            scenario->nodes[motion->node_index].id);
        fclose(mobility_file);
    } else
        return ERROR;
...
void motion_print(struct motion_class *motion) {
    printf(
        "  Motion: node_name='%s' type='%s' speed=(%.2f, %.2f, %.2f) \
destination=(%.2f, %.2f, %.2f) start_time=%.2f stop_time=%.2f\n",
        motion->node_name,
        motion_types[motion->type],
        motion->speed.c[0],
        motion->speed.c[1],
        motion->speed.c[2],
        motion->destination.c[0],
        motion->destination.c[1],
        motion->destination.c[2],
        motion->start_time,
        motion->stop_time);
...
void motion_copy(struct motion_class *motion_dst,
    ...
    // motion file name
    strncpy(motion_dst->mobility_filename,
            motion_src->mobility_filename,
            MAX_STRING - 1);
...
void motion_linear(struct motion_class *motion,
...
void motion_linear_acc(struct motion_class *motion,
    ...
    /*
       ...
       coordinate_vector_sum(&(node->position), &(node->position),
       &motion_vector);
     */
}
...
// for duration 'time_step'
void motion_circular(struct motion_class *motion,
                     struct node_class *node,
                     double time_step) {
...
void motion_rotation(struct motion_class *motion,
                     struct node_class *node,
                     double time_step) {
    int interf_j;
    ...
    // the antenna changes
    fprintf(stderr,
            "Applying rotation with angles: horiz=%f vert=%f\n",
...
void motion_random_walk(struct motion_class *motion,
                        struct node_class *node,
                        double time_step) {
...
void motion_levy_walk(struct motion_class *motion,
                      struct node_class *node,
                      double time_step) {
    // check if we need to make a change in velocity/direction
    // or whether this parameters are uninitialized (2nd condition)
...
void motion_homesick_levy_walk(struct motion_class *motion,
                               struct node_class *node,
                               double time_step) {
    // check if we need to make a change in velocity/direction
    // or whether this parameters are uninitialized (2nd condition)
...
int motion_behavioral_path_following(
    ...
    coordinate_init(
        &velocity_difference, "velocity_difference", 0.0, 0.0, 0.0);

    // compute distance to destination
    coordinate_vector_difference_2D(
        &path_vector, &(motion->destination), &(node->position));
    ...
    if (path_vector_magnitude > DESTINATION_DISTANCE_THRESHOLD) {
        ...
        coordinate_multiply_scalar(
            &path_vector_scaled, &path_vector, V0 / path_vector_magnitude);
        coordinate_print(&path_vector_scaled);

        // compute velocity difference
        coordinate_vector_difference(
            &velocity_difference, &path_vector_scaled, &(motion->speed));
        coordinate_print(&velocity_difference);

        // scale velocity difference by 'beta' and assign it to result
        coordinate_multiply_scalar(
            a_path_following, &velocity_difference, beta);
        ...
        coordinate_multiply_scalar(
            a_path_following, &path_vector_scaled, beta);
        coordinate_print(a_path_following);
#endif
...
double compute_detour_distance(struct object_class *object,
                               int vertex_index,
    ...
    while (counter < object->vertex_number) {
        if (go_clockwise == TRUE)
            ...
            vertex_i2 = ((vertex_i - 1) >= 0) ? vertex_i - 1
                                              : (object->vertex_number - 1);

        DEBUG(
            "vertex_i=%d vertex_i2=%d (go_clockwise=%d counter=%d); vertex=",
            vertex_i,
            vertex_i2,
            go_clockwise,
            counter);
        ...
        if (object_intersect(&(object->vertices[vertex_i]),
            ...
            detour_distance += coordinate_distance(
                &object->vertices[vertex_i], &object->vertices[vertex_i2]);
            printf("detour_distance=%f (partial result)\n", detour_distance);
        }
        // destination is visible
        else {
            detour_distance += coordinate_distance(
                &(object->vertices[vertex_i]), &destination);
            DEBUG("vertex=");
            coordinate_print(&(object->vertices[vertex_i]));
            printf(
                "---- return detour_distance=%f \
...
int motion_behavioral_object_avoidance(
    struct motion_class *motion,
    struct scenario_class *scenario,
    struct node_class *node,
    double time_step,
    struct coordinate_class *a_wall_rejection,
    struct coordinate_class *a_obstacle_avoidance) {
    ...
    for (object_i = 0; object_i < scenario->object_number; object_i++) {
        ...
        // we consider height is not important, and all objects must be
        // avoided

        // compute distance to all object edges
        last_vertex = (object->make_polygon == TRUE)
        ...
        for (vertex_i = 0; vertex_i < last_vertex; vertex_i++) {
            ...
            if (coordinate_distance_to_segment_2D(
                    &(node->position),
            ...
            if (min_distance < OBJECT_AVOIDANCE_THRESHOLD) {
                ...
                if (min_distance < WALL_REJECTION_THRESHOLD) {
                    if (avoid_corner == FALSE) {
                        ...
                        if (min_distance == 0) min_distance = 0.001;
                        coordinate_init(
                    ...
                    } else // this is not used for the moment
                    {
                        ...
                        coordinate_init(&acceleration_s,
                                        "acceleration_s",
        ...
        if (do_object_avoidance == TRUE) {
            ...
            // avoid the object by going around it (Rule 3: obstacle
            // avoidance)
            ...
            if (object_intersect(&(node->position),
                                 ...
                                 TRUE,
                                 FALSE) == FALSE) {
                // the obstacle doesn't prevent us from reaching the
                // destination therefore it doesn't have to be avoided; we can
                // skip the avoidance algorithm
                DEBUG("Object doesn't prevent reaching destination");
                continue; // skip to next object
            } else
                DEBUG(
                    "Object prevents reaching destination. \
Computing avoidance...");
            ...
            for (vertex_i = 0; vertex_i < last_vertex; vertex_i++) {
                ...
                // to compute the detour distance we need to compute the sum
                // of object edges up to last vertex from where the
                // destination is visible; this is done both clockwise and
                // counter-clockwise
                dist_clockwise = compute_detour_distance(
                    object, vertex_i, motion->destination, TRUE);

                dist_counter_clockwise = compute_detour_distance(
                    object, vertex_i, motion->destination, FALSE);
                /*
...
vertex_i=%d dist=%f dist_p_v=%f dist_c=%f dist_cc=%f\n",
            ...
            if (min_distance2 != DBL_MAX) {
                ...
                coordinate_vector_difference_2D(
                    &acceleration, &min_vertex, &(node->position));
                DEBUG("acceleration=");
                coordinate_print(&acceleration);
...
void motion_behavioral_mutual_avoidance(
    struct motion_class *motion,
    struct scenario_class *scenario,
    struct node_class *node,
    struct coordinate_class *a_mutual_avoidance) {
    // scaling constant
    double delta = 1.0;
    ...
    for (node_i = 0; node_i < scenario->node_number; node_i++) {
        ...
        if (distance < MUTUAL_AVOIDANCE_THRESHOLD) {
            ...
            if (magnitude > EPSILON) {
                ...
                printf(
                    "BEHAVIORAL: motion_id=%d angle=%f motion2->id=%d "
                    "angle2=%f\n",
                    motion->id,
                    angle,
                    (motion2 != NULL) ? motion2->id : -1,
                    angle2);

                // compute unit vector perpendicular on velocity
                if (angle < angle2)
                    // vector is rotated clockwise
                    coordinate_init(&unit_vector,
                                    "unit_vector",
                                    motion->speed.c[1],
                                    -motion->speed.c[0],
                                    motion->speed.c[2]);
                ...
                coordinate_multiply_scalar(
                    &unit_vector, &unit_vector, delta / magnitude);
                ...
                coordinate_multiply_scalar(
                    &unit_vector, &unit_vector, angle_difference);

                // multiply by the inverse of distance
                if (distance < 0.1) distance = 0.1;
                coordinate_multiply_scalar(
                    &unit_vector, &unit_vector, 1 / distance);
            }
            coordinate_copy(&local_avoidance, &unit_vector);
            printf("*** Mutual avoidance activated:\n");
            coordinate_print(&local_avoidance);

            coordinate_vector_sum(
                a_mutual_avoidance, a_mutual_avoidance, &local_avoidance);
        }
    }
}
...
int motion_behavioral(struct motion_class *motion,
    ...
    coordinate_init(&a_wall_rejection, "a_wall_rejection", 0.0, 0.0, 0.0);
    coordinate_init(
        &a_obstacle_avoidance, "a_obstacle_avoidance", 0.0, 0.0, 0.0);
    coordinate_init(&a_mutual_avoidance, "a_mutual_avoidance", 0.0, 0.0, 0.0);
    ...
    if (motion_behavioral_path_following(
            motion, node, time_step, &a_path_following) == TRUE) {
        // compute object rejection and avoidance acceleration
        motion_behavioral_object_avoidance(motion,
                                           scenario,
                                           node,
                                           time_step,
                                           &a_wall_rejection,
        ...
        motion_behavioral_mutual_avoidance(
            motion, scenario, node, &a_mutual_avoidance);
        ...
        if (coordinate_vector_magnitude(&a_wall_rejection) > EPSILON ||
            ...
            coordinate_init(
                &a_path_following, "a_path_following", 0.0, 0.0, 0.0);
        ...
        coordinate_print(&a_wall_rejection);
        coordinate_vector_sum(
            &acceleration, &a_path_following, &a_wall_rejection);
        ...
        coordinate_print(&a_obstacle_avoidance);
        coordinate_vector_sum(
            &acceleration, &acceleration, &a_obstacle_avoidance);
        ...
        coordinate_vector_sum(
            &acceleration, &acceleration, &a_mutual_avoidance);
        ...
        if (acceleration_magnitude > MAX_ACC_MAGN) {
            ...
            coordinate_multiply_scalar(&acceleration,
                                       &acceleration,
        ...
        if (speed_magnitude > MAX_SPEED_MAGN) {
            ...
            coordinate_multiply_scalar(
                &new_speed, &new_speed, MAX_SPEED_MAGN / speed_magnitude);
        ...
        coordinate_multiply_scalar(
            &acceleration, &acceleration, 1 / acceleration_magnitude);
    ...
    {
        INFO("Node '%s' reached destination %f s ago",
             node->name,
...
void motion_trace(struct motion_class *motion,
                  struct scenario_class *scenario,
                  struct node_class *node,
                  double motion_current_time,
                  double time_step) {
    ...
    if (motion_current_time >=
        ...
        if (motion->trace_record_crt < (motion->trace_record_number - 1)) {
            DEBUG(
                "Current motion time '%f' >= current trace record time '%f' => \
increment record index",
    ...
    DEBUG("Using linear motion with speed (%f,%f,%f)",
          motion->speed.c[0],
          motion->speed.c[1],
          motion->speed.c[2]);
...
int motion_apply(struct motion_class *motion,
    ...
    if (motion->node_index == INVALID_INDEX) // node could not be found
    {
        ...
        if (motion->type == LINEAR_MOTION && motion->speed_from_destination) {
            ...
            coordinate_vector_difference(
                &speed, &(motion->destination), &(node->position));

            DEBUG("Calculating speed from distance to destination");
            coordinate_multiply_scalar(
                &speed, &speed, 1 / (motion->stop_time - motion->start_time));
        ...
        else if (motion->type == LEVY_WALK_MOTION)
            motion_levy_walk(motion, node, time_step);
        else if (motion->type == HOMESICK_LEVY_WALK_MOTION)
            motion_homesick_levy_walk(motion, node, time_step);
        else if (motion->type == BEHAVIORAL_MOTION)
            motion_behavioral(motion, scenario, node, time_step);
        else if (motion->type == QUALNET_MOTION)
            motion_trace(
                motion, scenario, node, motion_current_time, time_step);
        else {
...
double motion_relative_velocity(struct scenario_class *scenario,
    ...
    if (motion1 != NULL) {
        ...
        if ((motion1->type == CIRCULAR_MOTION) ||
            ...
            WARNING(
                "Motion type '%s' is not supported for relative velocity \
calculation",
                motion_types[motion1->type]);
        ...
        // supported => check node 2
            ...
            if (motion2 != NULL) {
                ...
                if ((motion2->type == CIRCULAR_MOTION) ||
                    ...
                    WARNING(
                        "Motion type '%s' is not supported for relative \
velocity calculation",
                        motion_types[motion2->type]);
                    relative_velocity = -1.0;
                }
                ...
                // relative velocity by subtracting the speed vectors
                    ...
                    difference.c[0] =
                        motion1->speed.c[0] - motion2->speed.c[0];
                    difference.c[1] =
                        motion1->speed.c[1] - motion2->speed.c[1];
                    difference.c[2] =
                        motion1->speed.c[2] - motion2->speed.c[2];
                    relative_velocity =
...
velocity calculation",
        ...
        // supported, node 1 is static => only node 2 motion matters
            ...
            relative_velocity =
                coordinate_vector_magnitude(&(motion2->speed));
    } else
...
int motion_deltaQ(struct scenario_class *scenario,
                  double current_time,
                  double end_time,
    ...
    for (divider_i = 0; divider_i < motion_step_divider; divider_i++) {
        motion_current_time = current_time + divider_i * motion_step;

        if (motion_current_time > end_time) {
            break;
        }

// move nods according to the 'motions' object in 'scenario'
// for the next step of evaluation
#ifdef MESSAGE_INFO
        INFO("  node movement (sub-step %d)", divider_i);
        motion_found = FALSE;
#endif
        for (motion_i = start_i; motion_i < end_i; motion_i++)
            if ((motions[motion_i].start_time <= motion_current_time) &&
                (motions[motion_i].stop_time > motion_current_time)) {
                // fprintf(stderr, "\t\t\tcalculation => motion %d",
                // motion_i); fprintf(stderr, " \r");
                // temporary!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

                // if(motion_i!=45) continue;     // 0 corresponds to id 1

                if (motion_apply(&(motions[motion_i]),
                                 scenario,
                                 motion_current_time,
                                 motion_step) == ERROR) {
                    return ERROR;
