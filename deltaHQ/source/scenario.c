...
void scenario_init(struct scenario_class *scenario) {
    // 0 number of elements (nodes, objects, environments, motions,
    // connections)
...
void scenario_print(struct scenario_class *scenario) {
    printf(
        "Scenario: node_number=%d interface_number=%d object_number=%d \
...
void *scenario_add_environment(struct scenario_class *scenario,
    ...
    if (scenario->environment_number < MAX_ENVIRONMENTS) {
        environment_copy(
            &(scenario->environments[scenario->environment_number]),
            environment);
        return_value =
            &(scenario->environments[scenario->environment_number]);
        scenario->environment_number++;
...
void *scenario_add_connection(struct scenario_class *scenario,
    ...
    if (strcmp(connection->to_node, CONNECTION_TO_NODE_AUTO_CONNECT_STRING) ==
        0) {
        DEBUG(
            "Auto-connect request detected. The node '%s' will be \
            ...
            if (scenario->environments[env_i].name_hash ==
                ...
                if (strcmp(scenario->environments[env_i].name,
                    ...
                    /*
                       ...
                       {
                       WARNING ("ERROR: Environment '%s' is dynamic, and
                       cannot
                       be used to \
...
// reached (%d)!",
    ...
    // check to see if multiple to_node names are given
        ...
        DEBUG(
            "Connection to multiple nodes detected. Auto-generating \
connections...");
        ...
        for (env_i = 0; env_i < scenario->environment_number; env_i++) {
            if (scenario->environments[env_i].name_hash ==
                ...
                if (strcmp(scenario->environments[env_i].name,
                    ...
                    if (scenario->environments[env_i].is_dynamic == TRUE) {
                        WARNING(
                            "ERROR: Environment '%s' is dynamic, and cannot \
be used to define multiple connections",
                            connection->through_environment);
                        return NULL;
                    } else {
                        connection->through_environment_index = env_i;
        ...
        while (1) {
            ...
            if (scenario->connection_number < MAX_CONNECTIONS) {
                ...
                if (add_env_result != NULL ||
                    ...
                    WARNING(
                        "Could not add generated environment or incorrect \
environment index initialization");
                ...
                WARNING(
                    "Maximum number of connections in scenario reached \
(%d)!",
                    MAX_CONNECTIONS);
                return_value = NULL;
                break;
...
int scenario_auto_connect_nodes_at(struct scenario_class *scenario) {
    ...
    // neighbouring node
    for (node_i = 0; node_i < scenario->node_number; node_i++)
        if (node_auto_connect_at(&(scenario->nodes[node_i]), scenario) ==
            ERROR)
            return ERROR;
...
int scenario_init_state(struct scenario_class *scenario,
    ...
    TCHK_START(init_node_index);
    for (node_i = 0; node_i < scenario->node_number; node_i++) {
        // do initialization of object indexes
        if (node_init_interface_index(&(scenario->nodes[node_i]), scenario) ==
            ERROR) {
            WARNING("Error while initializing node interface indexes");
            return ERROR;
        }
    }
    TCHK_END(init_node_index);
    ...
    DEBUG(
        "* Environment validation and initialization done (%d "
        "environments)\n",
        scenario->environment_number);
    ...
    for (object_i = 0; object_i < scenario->object_number; object_i++) {
        // do initialization of object indexes
        if (object_init_index(&(scenario->objects[object_i]), scenario) ==
            ERROR) {
    ...
    if (jpgis_filename_provided == TRUE)
        if (xml_jpgis_load_objects(scenario,
                                   scenario->objects,
                                   scenario->object_number,
    ...
    // remove region place-holder objects after JPGIS objects were added (if
    // any)
    DEBUG("Check objects in scenario");
    for (object_i = 0; object_i < scenario->object_number; object_i++) {
        if (scenario->objects[object_i].load_all_from_region == TRUE) {
            DEBUG(
                "Remove region place-holder object '%s' after adding JPGIS "
                "objects",
    ...
    while (1) {
        ...
        for (object_i = 0; object_i < scenario->object_number; object_i++) {
            ...
            if ((fabs(crt_object->vertices[0].c[0] -
                      crt_object->vertices[crt_object->vertex_number - 1]
                          .c[0]) > EPSILON) ||
                (fabs(crt_object->vertices[0].c[1] -
                      crt_object->vertices[crt_object->vertex_number - 1]
                          .c[1]) > EPSILON)) {
                if (crt_object->make_polygon == TRUE) {
                    INFO(
                        "The polyline object '%s' is considered a polygon; \
no merging necessary",
                    ...
                    DEBUG(
                        "The object '%s' is not a polygon. Trying to "
                        "merge...",
                        crt_object->name);
        ...
        if (tested_objects == scenario->object_number) break;
    }
    ...
    TCHK_START(init_motion_index);
    // for (motion_i = 0; motion_i < scenario->motion_number;
    //      motion_i++) { // do initialization of motion indexes
    //     if (motion_init_index(&(scenario->motions[motion_i]),
    //                           scenario,
    //                           cartesian_coord_syst) == ERROR) {
    //         WARNING("Error while initializing motion indexes");
    //         return ERROR;
    //     }
    // }
    TCHK_END(init_motion_index);
    ...
    TCHK_START(init_conn_index);

    pthread_t *pt = (pthread_t *) malloc(sizeof(pthread_t) * thread_number);
    struct array_range ary_range;

    ary_range.loop = scenario->connection_number;
    ary_range.step = scenario->connection_number / thread_number;
    ary_range.mod  = scenario->connection_number % thread_number;

    struct thread_connection_class *thread_conn =
        (struct thread_connection_class *) malloc(
            sizeof(struct thread_connection_class) * thread_number);
#ifndef __APPLE__
    cpu_set_t cpu;
#endif
    ...
    for (int i = 0; i < thread_number; i++) {
        ...
        CPU_ZERO(&cpu);
        CPU_SET(i, &cpu);
        pthread_setaffinity_np(pt[i], sizeof(cpu_set_t), &cpu);
#endif

        thread_get_index(&ary_range);

        DEBUG("start=%d end=%d", ary_range.start, ary_range.end);

        thread_conn[i].scenario = scenario;
        thread_conn[i].start    = ary_range.start;
        thread_conn[i].end      = ary_range.end;

        pthread_create(
            &pt[i], NULL, thread_connection_init_index, &thread_conn[i]);
    }
    ...
    for (int i = 0; i < thread_number; i++) {
        ...
        pthread_create(
            &pt[i], NULL, thread_connection_init_status, &thread_conn[i]);
    }
...
void *scenario_deltaQ(void *arg) {
    struct scenario_deltaQ_class *p = (struct scenario_deltaQ_class *) arg;
    ...
    pthread_getname_np(pthread_self(), name, sizeof(char) * NAME_SIZE);
    index = atoi(name);

#ifdef __APPLE__
    pthread_mutex_lock(p->mut_set_index);
    index = global_index;
    global_index++;
    pthread_mutex_unlock(p->mut_set_index);
#endif

    start_conn = p->start_conn[index];
    end_conn   = p->end_conn[index];
    ...
    while (1) {
        ...
        for (connection_i = start_conn; connection_i < end_conn;
             connection_i++) {

            connection = p->neighbor[connection_i];
...
int scenario_try_merge_object(struct scenario_class *scenario,
                              int merge_object_i) {
    int crt_object_j;
    ...
    for (crt_object_j = 0; crt_object_j < scenario->object_number;
        ...
        if (merge_object_i == crt_object_j) continue;
        ...
        if ((fabs(merge_object->vertices[0].c[0] -
            ...
            if (scenario_merge_objects(
                    scenario, crt_object_j, merge_object_i, TRUE) == SUCCESS)
                return TRUE;
            else
                return FALSE;
        }
        // first vertex equals first vertex!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            INFO(
                "Merging object '%s' to object '%s' by direct appending "
                "after "
                "reversing...",
                merge_object->name,
                crt_object->name);
        ...
        else if ((fabs(merge_object->vertices[merge_object->vertex_number - 1]
                           .c[0] -
                       crt_object->vertices[0].c[0]) < EPSILON) &&
                 (fabs(merge_object->vertices[merge_object->vertex_number - 1]
                           .c[1] -
                       crt_object->vertices[0].c[1]) < EPSILON)) {
            INFO("Merging object '%s' to object '%s' by direct appending...",
                 crt_object->name,
                 merge_object->name);
            if (scenario_merge_objects(
                    scenario, merge_object_i, crt_object_j, TRUE) == SUCCESS)
                return TRUE;
            else
                return FALSE;
        }
        // last vertex equals last vertex
                 ...
                 (fabs(merge_object->vertices[merge_object->vertex_number - 1]
                       ...
                       crt_object->vertices[crt_object->vertex_number - 1]
                           .c[1]) < EPSILON)) {
            INFO(
                "Merging object '%s' to object '%s' by reverse "
                "appending...",
                crt_object->name,
                merge_object->name);
            if (scenario_merge_objects(
                    scenario, merge_object_i, crt_object_j, FALSE) == SUCCESS)
                return TRUE;
            else
                return FALSE;
        }
...
int scenario_merge_objects(struct scenario_class *scenario,
    ...
    if (direct_merge == TRUE) {
        ...
        if ((fabs(object1->vertices[0].c[0] - object2->vertices[0].c[0]) <
             EPSILON) &&
            (fabs(object1->vertices[0].c[1] - object2->vertices[0].c[1]) <
             EPSILON)) {
    ...
    strncat(
        object1->name, object2->name, MAX_STRING - strlen(object1->name) - 1);
