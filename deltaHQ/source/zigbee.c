  ...
  if (environment->num_segments > 0) {
    ...
    if (environment->num_segments == 1) {
      ...
      connection->Pr =
          ...
          10 * environment->alpha[0] * log10(distance) - environment->W[0] +
          randn(0, environment->sigma[0]) +
...
int zigbee_compute_channel_interference(struct connection_class *connection,
  ...
  adapter = zigbee_get_interface_adapter(
      connection, (&((scenario->nodes[connection->from_node_index])
...
int zigbee_fer(struct connection_class *connection,
  ...
  if (adapter->use_model1 == TRUE) {
    fer1 =
        adapter->Pr_threshold_fer *
        exp(adapter->model1_alpha * (adapter->Pr_thresholds[operating_rate] -
    ...
    DEBUG("fer1=%f Pr_threshold_fer=%f model1_alpha=%f "
          "Pr_thresholds[operating_rate]=%f SNR=%f ZIGBEE_STANDARD_NOISE=%f",
    ...
    // INCORRECT
    fer1 = 1 - pow(1 - fer1, (connection->packet_size + header) / ZIGBEE_PSDU);
...
int zigbee_ppdu_duration(struct connection_class *connection,
  ...
  (*ppdu_duration) = PHY_Overhead + MAC_Overhead + payload_duration +
                     delay_to_ACK + Preamble_duration + SFD_duration +
                     ACK_duration + IFS_duration;
...
int zigbee_delay_jitter(struct connection_class *connection,
  ...
  if (connection->delay_defined == FALSE) {
    ...
    DEBUG("consider_interference=%d concurrent_stations=%d",
          connection->consider_interference, connection->concurrent_stations);
