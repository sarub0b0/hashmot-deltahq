...
// init a node
void node_init(struct node_class *node,
               const char *name,
               int type,
               int id,
               const char *ssid,
...
void node_copy(struct node_class *node_dst, struct node_class *node_src) {
    ...
    for (i = 0; i < node_src->interface_number; i++)
        interface_copy(&(node_dst->interfaces[i]),
                       &(node_src->interfaces[i]));
...
int node_auto_connect(struct node_class *from_node,
    ...
    if (scenario->connection_number < MAX_CONNECTIONS) {
        ...
        for (i = 0; i < scenario->node_number; i++)
            if (scenario->nodes[i].id == from_node->id) from_node_index = i;
        ...
        for (i = 0; i < scenario->node_number; i++)
            ...
            if (scenario->nodes[i].id != from_node->id) {
                ...
                if (scenario->nodes[i].type == ACCESS_POINT_NODE) {
                    // only from_nodes with connection "infrastructure" or
                    // "any" can connect to an access point
                    if ((scenario->nodes[from_node_index].connection ==
                         INFRASTRUCTURE_CONNECTION) ||
                        (scenario->nodes[from_node_index].connection ==
                         ANY_CONNECTION))
                        // the "ssid" must be the same with that of the access
                        // point
                ...
                // assume the potential to_node is a regular node
                    ...
                    if ((scenario->nodes[from_node_index].connection ==
                        ...
                        (scenario->nodes[from_node_index].connection ==
                         ANY_CONNECTION))
                        // the "ssid" must be the same with that of the other
                        // node
                        if (strcmp(scenario->nodes[from_node_index].ssid,
                                   scenario->nodes[i].ssid) == 0)
                            try_connect = TRUE;
                ...
                if (try_connect == TRUE) {
                    // set the attribute 'to_node' of the connection
                    // temporarily to the current node
        ...
        if (max_Pr_index != INVALID_INDEX) {
            ...
            strncpy(connection->to_node,
                    scenario->nodes[max_Pr_index].name,
                    MAX_STRING - 1);
    ...
    for (i = 0; i < scenario->node_number; i++) {
        ...
        if (scenario->nodes[i].id != from_node->id) {
            if (scenario->connection_number < MAX_CONNECTIONS) {
                ...
                // make sure all parameters are initialized to default values
                if (connection_init(candidate_connection,
                                    DEFAULT_PARAMETER,
                                    DEFAULT_PARAMETER,
                                    DEFAULT_PARAMETER,
                                    7,
                ...
                active_tag_loss_rate(candidate_connection,
                                     scenario,
...
int node_check_valid(struct node_class *nodes,
                     int node_number,
    ...
    for (i = 0; i < node_number; i++) {
        ...
        /* This code is not required anymore since ids are generated
           automatically
...
int node_add_interface(struct node_class *node,
    ...
    if (node->interface_number >= MAX_INTERFACES) {
        WARNING("Maximum number of interfaces (%d) exceeded.",
                MAX_INTERFACES);
        return ERROR;
    } else {
        interface_copy(&(node->interfaces[node->interface_number]),
                       interface);
        node->interface_number++;
    }
...
int node_init_interface_index(struct node_class *node,
    ...
    DEBUG(
        "Initialized %d interface(s) for node %d (global interface "
        "number=%d)",
        node->interface_number,
        node->id,
        scenario->interface_number);
...
int node_init_arrival_distance(struct node_class *nodes, int node_number) {

    // struct node_class node;
    struct node_class tx, rx;
