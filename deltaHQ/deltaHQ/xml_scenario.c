...
#define DOUBLE_CMP(a, b) (fabs(a - b) < DBL_EPSILON)
...
purposes.");
            ...
            if (DOUBLE_CMP(double_result, -HUGE_VAL))
                return ERROR;
            else
...
static void XMLCALL character_data(void *data,
    ...
    if (strlen(xml_scenario->buffer) + (unsigned long) length < MAX_STRING) {
        // printf("--DEB (bef): '%s'\n", xml_scenario->buffer);
        strncat(xml_scenario->buffer, string, (unsigned long) length);
        // printf("DEB (after): '%s'\n", xml_scenario->buffer);
    } else {
