...
static void usage(FILE *f) {
    ...
    fprintf(f,
            ...
            "text output)\n");
    fprintf(f,
            " -n, --no-deltaQ        - NO text NOR binary deltaQ output will "
            "be written\n");
    fprintf(f,
    ...
    fprintf(f,
            " -d, --disable-deltaQ   - disable deltaQ computation (output "
            "still generated)\n");
    fprintf(f, "\n");
...
long int parse_svn_revision(char *svn_revision_str) {
    ...
    number] 4168M         modified working copy        [=> use number part]
...
int main(int argc, char **argv) {
        ...
        switch (c) {
            ...
            case 'm':
                ...
                if (motion_output_enabled == TRUE) {
                    WARNING(
                        "Multiple arguments related to motion output "
                        "detected.");
                    WARNING(
                        "Note that arguments '-m' and '-s' are mutually "
                        "exclusive.");
                    usage(stdout);
                    exit(1);
                } else {
    ...
    if ((text_only_enabled == TRUE && binary_only_enabled == TRUE) ||
        ...
        WARNING(
            "The output control options 'text-only', 'binary-only', and \
    ...
    if (io_connection_state == NULL) {
        ...
        printf(
            "MEMORY: Allocated %.2f MB (%zd bytes) for connection state "
            "storage.\n",
            sizeof(io_cls) / 1048576.0,
            sizeof(io_cls));
    ...
    if (!(text_only_enabled || binary_only_enabled)) {
        ...
        if (strlen(settings_filename) > MAX_STRING - 10) {
            WARNING(
                "Cannot create settings file name because input \
                    filename '%s' exceeds %d characters!",
                settings_filename,
                MAX_STRING - 10);
            goto ERROR_HANDLE;
        }
    ...
    if (binary_output_enabled == TRUE) {
        ...
        if (strlen(binary_output_filename) > MAX_STRING - 5) {
            WARNING(
                "Cannot create binary output file name because input \
                    filename '%s' exceeds %d characters!",
        ...
        strncat(binary_output_filename,
                ".bin",
        ...
        io_binary_write_header_to_file(scenario->interface_number,
                                       0,
                                       MAJOR_VERSION,
                                       MINOR_VERSION,
                                       SUBMINOR_VERSION,
    ...
    if (text_output_enabled == TRUE) {
        ...
        if (strlen(text_output_filename) > MAX_STRING - 5) {
            WARNING(
                "Cannot create output file name because input filename \
    ...
    if (motion_output_enabled == TRUE) {
        ...
        if (strlen(motion_filename) > MAX_STRING - 5) {
            WARNING(
                "Cannot create motion file name because input filename \
                    '%s' exceeds %d characters!",
                output_filename_base,
                MAX_STRING - 5);
            goto ERROR_HANDLE;
    ...
    if (object_output_enabled == TRUE) {
        ...
        if (strlen(object_output_filename) > MAX_STRING - 5) {
            WARNING(
                "Cannot create object output file name because input \
                    filename '%s' exceeds %d characters!",
                output_filename_base,
                MAX_STRING - 5);
            goto ERROR_HANDLE;
        ...
        strncat(object_output_filename,
                ".obj",
        ...
        // write object output
        io_write_objects(
            scenario, xml_scenario->cartesian_coord_syst, object_output_file);
    ...
    for (int node_i = 0; node_i < scenario->node_number; node_i++) {
        ...
        for (int if_i = 0; if_i < node->interface_number; if_i++) {
            node->interfaces[if_i].Pt_arrival_distance = 17.0;
        }
    }

    struct calculation_class *calc = &(scenario->calculations);
    ////////////////////////////////////////////////////////////
    // processing phase
    ...
    for (current_time = xml_scenario->start_time;
        ...
        if (motion_output_enabled == TRUE) {
            if (current_time == xml_scenario->start_time) {
                ...
                if (motion_output_type == MOTION_OUTPUT_NAM) {
                    ...
                    // io_write_ns2_motion_header_to_file (scenario,
                    // motion_file);
                    io_write_ns2_motion_header_to_file_new(
                        scenario, motion_file, current_time);
                }
            } else {
                // write motion info
                if (motion_output_type == MOTION_OUTPUT_NAM)
                    io_write_nam_motion_info_to_file(
                        scenario, motion_file, current_time);
                else
                    // io_write_ns2_motion_info_to_file (scenario,
                    // motion_file, current_time);
                    io_write_ns2_motion_info_to_file_new(
                        scenario, motion_file, current_time);
            }
        }
        ...
        for (int conn_i = 0; conn_i < scenario->connection_number; conn_i++) {
            ...
            distance = coordinate_distance(&rx->position, &tx->position);

            if (distance < tx->interfaces[conn->from_interface_index]
                               .Pt_arrival_distance) {
                // scenario->calculations
                calc->connection_indexes[calc->calculation_number] = conn_i;
        ...
        printf(
            "calc_length: %.1f %d\n", current_time, calc->calculation_number);
        ...
        for (connection_i = 0; connection_i < scenario->connection_number;
             connection_i++) {
            if (own_id != -1) {
                if (scenario->connections[connection_i].from_node_index !=
                        own_id &&
            ...
            if (scenario->nodes[connection->from_node_index]
                ...
                if (current_time == xml_scenario->start_time) {
                    ...
                    if (io_binary_compare_record(
                        ...
                        io_binary_build_record(
                            ...
                            &(scenario->connections[connection_i]),
                            scenario);
                        io_connection_state->state_changed[connection_i] =
                            TRUE;
        ...
        if (binary_output_enabled == TRUE) {
            ...
            if (io_connection_state->binary_time_record.record_number > 0) {
                ...
                for (connection_i = 0;
                    ...
                    if (io_connection_state->state_changed[connection_i] ==
                        TRUE) {
                        ...
                        if (record_i < io_connection_state->binary_time_record
                                           .record_number) {
                            record_i++;
                        } else {
                            break;
                ...
                if (record_i <
                    io_connection_state->binary_time_record.record_number) {
                    WARNING(
                        "At time %f wrote ONLY %d binary records out of %d",
                        current_time,
                        record_i,
                        io_connection_state->binary_time_record
                            .record_number);
                } else {
                    INFO("At time %f wrote %d binary records",
                         current_time,
                         record_i);
                }
#ifdef DISABLE_EMPTY_TIME_RECORDS
        ...
        for (divider_i = 0; divider_i < xml_scenario->motion_step_divider;
            ...
            if (motion_current_time > (xml_scenario->duration +
                                       xml_scenario->start_time + EPSILON)) {
                break;
            }
            ...
            for (motion_i = 0; motion_i < scenario->motion_number; motion_i++)
                if ((scenario->motions[motion_i].start_time <=
                     motion_current_time) &&
                    (scenario->motions[motion_i].stop_time >
                     motion_current_time)) {
                    // fprintf (stderr, "\t\t\tcalculation => motion %d \r",
                    // motion_i); TEMPORARY!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

                    // if(motion_i!=45) continue;     // 0 corresponds to id 1

                    if (motion_apply(&(scenario->motions[motion_i]),
                                     scenario,
                                     motion_current_time,
                                     motion_step) == ERROR) {
                        goto ERROR_HANDLE;
...
FINAL_HANDLE:
    ...
    if (error_status == SUCCESS) {
        ...
        INFO(
            "\n-- Scenario processing completed with errors (see the WARNING "
            "messages above)\n\n");
        fprintf(stderr,
