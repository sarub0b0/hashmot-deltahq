...
long int long_int_value(const char *string) {
    ...
    if ((string[0] == '\0') || (end_pointer[0] != '\0') || (errno != 0)) {
        perror("strtol");
        WARNING(
            "Converting value '%s' to long int failed! %d", string, errno);
        return LONG_MIN;
    }
...
double rand_min_max_inclusive_power_law(double min, double max, double beta) {
...
double antenna_azimuth_attenuation(struct node_class *node_tx,
    ...
    if (interface_tx->azimuth_beamwidth == 360)
        ...
        angle_tx_rx = atan2(node_rx->position.c[1] - node_tx->position.c[1],
                            node_rx->position.c[0] - node_tx->position.c[0]) *
                      180 / M_PI;
        ...
        angle_diff = (angle_diff > 360.0 - angle_diff) ? 360.0 - angle_diff
                                                       : angle_diff;
        ...
        if (angle_diff > interface_tx->azimuth_beamwidth / 2)
            ...
            directional_attenuation =
                3 * 2 * angle_diff / interface_tx->azimuth_beamwidth;
    }

    /*
       DEBUG("*_*_*_*_*_*_*_  rx_y=%f tx_y=%f rx_x=%f tx_x=%f a_t_r=%f a_o=%f
       a_d=%f a_b=%f d_g=%f", node_rx->position_y, node_tx->position_y,
       node_rx->position_x, node_tx->position_x, angle_tx_rx,
       node_tx->azimuth_orientation, angle_diff, node_tx->azimuth_beamwidth,
       directional_attenuation);
     */
...
double antenna_elevation_attenuation(struct node_class *node_tx,
    ...
    if (interface_tx->elevation_beamwidth == 360)
        ...
        angle_tx_rx = atan2(node_rx->position.c[2] - node_tx->position.c[2],
                            node_rx->position.c[0] - node_tx->position.c[0]) *
                      180 / M_PI;
        ...
        if (angle_diff > interface_tx->elevation_beamwidth / 2)
            ...
            // 3 dB is attenuation at beamwidth/2 angle
            directional_attenuation =
                3 * 2 * angle_diff / interface_tx->elevation_beamwidth;
    }
