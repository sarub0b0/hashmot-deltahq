                ...
                if (i > 0) {
                    ...
                    if (time < motion->trace_records[i - 1].time) {
                        WARNING(
                            ...
                            "of the last added record (%f). Aborting...",
                            time,
...
void motion_copy(struct motion_class *motion_dst,
    ...
    // levy walk fields
    motion_dst->alpha = motion_src->alpha;
    motion_dst->beta  = motion_src->beta;
...
void motion_levy_walk(struct motion_class *motion,
    ...
    if (motion->current_walk_time >= motion->walk_time ||
        ...
        motion->walk_time =
            rand_min_max_inclusive_power_law(5.0, 20.0, motion->beta);
...
void motion_homesick_levy_walk(struct motion_class *motion,
    ...
    if (motion->current_walk_time >= motion->walk_time ||
        ...
        // generate a new velocity between min_speed and max_speed
        motion->velocity = rand_min_max_inclusive_power_law(
            motion->min_speed, motion->max_speed, motion->beta);
