...
int xml_node_init(struct node_class *node,
    ...
    interface_init(&(node->interfaces[0]),
        ...
        if (strcmp(attributes[i], NODE_NAME_STRING) == 0) {
            ...
            if (strcmp(attributes[i + 1], NODE_TYPE_REGULAR_STRING) == 0)
                node->type = REGULAR_NODE;
                            ...
                            NODE_TYPE_ACCESS_POINT_STRING) == 0)
            ...
            WARNING(
                "Node attribute 'id' is not supported anymore, as the id \
...
purposes.");
            ...
            /*
               ...
               return ERROR;
               else if ((long_int_result >= INT_MIN) && (long_int_result <=
               INT_MAX)) node->id = (int) long_int_result; else
               {
               ...
               Ignored negative value ('%s')", NODE_ID_STRING, attributes[i +
               1]); node->id = 0; return ERROR;
               }
               else
               id_provided = TRUE;
             */
            ...
            else if (strcmp(attributes[i + 1], NODE_CONNECTION_ANY_STRING) ==
                     0)
            ...
            if (strcmp(attributes[i + 1], INTERFACE_ADAPTER_ORINOCO_STRING) ==
                0)
                ...
                node->interfaces[0].adapter_type = CISCO_ABG;
                            ...
                            INTERFACE_ADAPTER_JENNIC_STRING) == 0)
                node->interfaces[0].adapter_type = JENNIC;
            else if (strcmp(attributes[i + 1],
                            INTERFACE_ADAPTER_S_NODE_STRING) == 0)
        ...
        } else if (strcmp(attributes[i], INTERFACE_ANTENNA_GAIN_STRING) ==
                   0) {
            node->interface_number =
                1; // interface attribute was set => define interface
            ...
            if (double_result == -HUGE_VAL)
                ...
                node->interfaces[0].antenna_gain = double_result;
                          ...
                          INTERFACE_AZIMUTH_ORIENTATION_STRING) == 0) {
            node->interface_number =
                1; // interface attribute was set => define interface
            ...
            if (double_result == -HUGE_VAL)
                ...
                node->interfaces[0].azimuth_orientation = double_result;
                          ...
                          INTERFACE_AZIMUTH_BEAMWIDTH_STRING) == 0) {
            node->interface_number =
                1; // interface attribute was set => define interface
            ...
            if (double_result == -HUGE_VAL)
                ...
                node->interfaces[0].azimuth_beamwidth = double_result;
                          ...
                          INTERFACE_ELEVATION_ORIENTATION_STRING) == 0) {
            node->interface_number =
                1; // interface attribute was set => define interface
            ...
            if (double_result == -HUGE_VAL)
                ...
                node->interfaces[0].elevation_orientation = double_result;
                          ...
                          INTERFACE_ELEVATION_BEAMWIDTH_STRING) == 0) {
            node->interface_number =
                1; // interface attribute was set => define interface
        ...
        } else if (strcmp(attributes[i], INTERFACE_ANTENNA_COUNT_STRING) ==
                   0) {
            node->interface_number =
                1; // interface attribute was set => define interface
            ...
            if (long_int_result == LONG_MIN)
                ...
                WARNING(
                    "Node attribute '%s' has a value outside the 'int' \
range ('%ld')",
                    NODE_ID_STRING,
                    long_int_result);
                return ERROR;
            ...
            if (node->interfaces[0].antenna_count < 1 ||
                ...
                WARNING(
                    "Interface attribute '%s' must be in the range [%d,%d]. \
Ignored incorrect value ('%s')",
                    INTERFACE_ANTENNA_COUNT_STRING,
                    1,
                    MAX_ANTENNA_COUNT,
                    attributes[i + 1]);
            ...
            if (node->internal_delay < 0) {
                WARNING(
                    "Node attribute '%s' must be a positive double. \
Ignored negative value ('%s')",
                    NODE_INTERNAL_DELAY_STRING,
                    attributes[i + 1]);
...
int xml_interface_init(struct interface_class *interface,
                       const char **attributes) {
    int i;
...
Ignored incorrect value ('%s')",
            ...
            strncpy(
                interface->ip_address, attributes[i + 1], IP_ADDR_SIZE - 1);
            interface->ip_address[IP_ADDR_SIZE - 1] = '\0';
        } else if (strcmp(attributes[i], INTERFACE_NOISE_SOURCE_STRING) ==
                   0) {
            if (strcmp(attributes[i + 1], TRUE_STRING) == 0)
                ...
                WARNING(
                    "Interface attribute '%s' is assigned an invalid value \
('%s')",
                    INTERFACE_NOISE_SOURCE_STRING,
                    attributes[i + 1]);
                return ERROR;
            }
        } else if (strcmp(attributes[i], INTERFACE_NOISE_START_TIME_STRING) ==
                   0) {
            double_result = double_value(attributes[i + 1]);
            if (double_result == -HUGE_VAL)
                return ERROR;
        ...
        } else if (strcmp(attributes[i], INTERFACE_NOISE_END_TIME_STRING) ==
                   0) {
            double_result = double_value(attributes[i + 1]);
            if (double_result == -HUGE_VAL)
                return ERROR;
...
// cartesian coordinates are used or not;
// return SUCCESS on succes, ERROR on error
int xml_object_init(struct object_class *object,
                    const char **attributes,
                    int cartesian_coord_syst) {
    int i;
    ...
    for (i = 0; attributes[i]; i += 2)
        if (strcmp(attributes[i], OBJECT_NAME_STRING) == 0)
            ...
            if (strcmp(attributes[i + 1], OBJECT_TYPE_BUILDING_STRING) == 0)
                ...
                WARNING(
                    "Object attribute '%s' is assigned an invalid value \
('%s')",
                    OBJECT_TYPE_STRING,
                    attributes[i + 1]);
                return ERROR;
...
('%s')",
            ...
            if (double_result == -HUGE_VAL)
                ...
                object->height = double_result;
                          ...
                          OBJECT_LOAD_FROM_JPGIS_FILE_STRING) == 0) {
            if (strcmp(attributes[i + 1], TRUE_STRING) == 0)
                ...
                WARNING(
                    "Object attribute '%s' is assigned an invalid value \
('%s')",
                    OBJECT_LOAD_FROM_JPGIS_FILE_STRING,
                    attributes[i + 1]);
                return ERROR;
            }
        } else if (strcmp(attributes[i],
                          OBJECT_LOAD_ALL_FROM_REGION_STRING) == 0) {
            if (strcmp(attributes[i + 1], TRUE_STRING) == 0)
                ...
                WARNING(
                    "Object attribute '%s' is assigned an invalid value \
('%s')",
                    OBJECT_LOAD_ALL_FROM_REGION_STRING,
                    attributes[i + 1]);
                return ERROR;
            }
        } else {
...
int xml_environment_init(struct environment_class *environment,
    ...
    environment_init(environment,
                     ...
                     DEFAULT_ENVIRONMENT_COORD_1,
                     DEFAULT_ENVIRONMENT_COORD_1,
                     DEFAULT_ENVIRONMENT_COORD_2,
                     DEFAULT_ENVIRONMENT_COORD_2);
    ...
    for (i = 0; attributes[i]; i += 2)
        if (strcmp(attributes[i], ENVIRONMENT_NAME_STRING) == 0) {
            ...
            if (strcmp(attributes[i + 1], TRUE_STRING) == 0)
                ...
                WARNING(
                    "Environment attribute '%s' is assigned an invalid \
value ('%s')",
                    ENVIRONMENT_IS_DYNAMIC_STRING,
                    attributes[i + 1]);
                return ERROR;
            }
            ...
            if (environment->alpha[0] < 0) {
                WARNING(
                    "Environment attribute '%s' must be a positive double. \
Ignored negative value ('%s')",
                    ENVIRONMENT_ALPHA_STRING,
                    attributes[i + 1]);
...
Ignored negative value ('%s')",
            ...
            if (environment->W[0] < 0) {
                WARNING(
                    "Environment attribute '%s' must be a positive double. \
Ignored negative value ('%s')",
                    ENVIRONMENT_W_STRING,
                    attributes[i + 1]);
                environment->W[0] = 0;
                return ERROR;
            }
        } else if (strcmp(attributes[i], ENVIRONMENT_NOISE_POWER_STRING) ==
                   0) {
            double_result = double_value(attributes[i + 1]);
            if (double_result == -HUGE_VAL)
                return ERROR;
            ...
            if (strcmp(attributes[i + 1], ENVIRONMENT_FADING_AWGN_STRING) ==
                0)
                ...
                WARNING(
                    "Environment attribute '%s' is assigned an invalid value \
('%s')",
                    ENVIRONMENT_FADING_STRING,
                    attributes[i + 1]);
                return ERROR;
            }
        } else {
...
// cartesian coordinates are used or not;
// return SUCCESS on succes, ERROR on error
int xml_motion_init(struct motion_class *motion,
                    const char **attributes,
                    int cartesian_coord_syst) {
    int i;
    ...
    motion_init(motion,
                INVALID_INDEX,
                DEFAULT_MOTION_NODE_NAME,
                DEFAULT_MOTION_TYPE,
                DEFAULT_MOTION_SPEED,
                DEFAULT_MOTION_SPEED,
                DEFAULT_MOTION_SPEED,
                DEFAULT_MOTION_CENTER,
                DEFAULT_MOTION_CENTER,
                DEFAULT_MOTION_SPEED,
                DEFAULT_MOTION_SPEED,
                DEFAULT_MOTION_MAX_SPEED,
                DEFAULT_MOTION_WALK_TIME,
                DEFAULT_MOTION_ALPHA,
                DEFAULT_MOTION_BETA,
                DEFAULT_MOTION_TIME,
                DEFAULT_MOTION_TIME);
    ...
    for (i = 0; attributes[i]; i += 2)
        if (strcmp(attributes[i], MOTION_NODE_NAME_STRING) == 0) {
            ...
            else if (strcmp(attributes[i + 1], MOTION_TYPE_CIRCULAR_STRING) ==
                     0)
                motion->type = CIRCULAR_MOTION;
            else if (strcmp(attributes[i + 1], MOTION_TYPE_ROTATION_STRING) ==
                     0)
                motion->type = ROTATION_MOTION;
            else if (strcmp(attributes[i + 1],
                            MOTION_TYPE_RANDOM_WALK_STRING) == 0)
                motion->type = RANDOM_WALK_MOTION;
            else if (strcmp(attributes[i + 1],
                            MOTION_TYPE_LEVY_WALK_STRING) == 0)
                motion->type = LEVY_WALK_MOTION;
            else if (strcmp(attributes[i + 1],
                            MOTION_TYPE_HOMESICK_LEVY_WALK_STRING) == 0)
                motion->type = HOMESICK_LEVY_WALK_MOTION;
            else if (strcmp(attributes[i + 1],
                            MOTION_TYPE_BEHAVIORAL_STRING) == 0)
                motion->type = BEHAVIORAL_MOTION;
            else if (strcmp(attributes[i + 1], MOTION_TYPE_QUALNET_STRING) ==
                     0)
                motion->type = QUALNET_MOTION;
            else {
                WARNING(
                    "Motion attribute '%s' is assigned an invalid value \
('%s')",
                    MOTION_TYPE_STRING,
                    attributes[i + 1]);
                return ERROR;
        ...
        } else if (strcmp(attributes[i], MOTION_ALPHA_STRING) == 0) {
            double_result = double_value(attributes[i + 1]);
            if (double_result == -HUGE_VAL)
                return ERROR;
            else
                motion->alpha = double_result;
        } else if (strcmp(attributes[i], MOTION_BETA_STRING) == 0) {
            double_result = double_value(attributes[i + 1]);
            if (double_result == -HUGE_VAL)
                return ERROR;
            else
                motion->beta= double_result;
        ...
        } else if (strcmp(attributes[i], MOTION_MOBILITY_FILENAME_STRING) ==
                   0) {
            strncpy(
                motion->mobility_filename, attributes[i + 1], MAX_STRING - 1);
    ...
    if (motion->type == LINEAR_MOTION) {
        if (speed_x_provided == FALSE && speed_y_provided == FALSE &&
            speed_z_provided == FALSE && destination_x_provided == FALSE &&
            destination_y_provided == FALSE &&
            destination_z_provided == FALSE) {
            WARNING(
                "For linear motion, either 'speed_x/y/z' or \
'destination_x/y/z' atributes must be initialized");
            return ERROR;
        } else if (speed_x_provided == TRUE && speed_y_provided == TRUE &&
                   speed_z_provided == TRUE &&
                   destination_x_provided == TRUE &&
                   destination_y_provided == TRUE &&
                   destination_z_provided == TRUE) {
            WARNING(
                "For linear motion, not both 'speed_x/y/z' and \
'destination_x/y/z' can be initialized");
            return ERROR;
        } else if (speed_x_provided == FALSE && speed_y_provided == FALSE &&
...
'destination_x/y/z' have to be provided");
        ...
        if (center_x_provided == FALSE || center_y_provided == FALSE) {
            WARNING(
                "For circular motion, attributes 'center_x/y' \
must be initialized");
            return ERROR;
        }
    } else if (motion->type == BEHAVIORAL_MOTION) {
        if (destination_x_provided == FALSE ||
            destination_y_provided == FALSE ||
            destination_z_provided == FALSE) {
            WARNING(
                "For behavioral motion, attributes 'destination_x/y/z' \
must be initialized");
            return ERROR;
        } else
            motion->speed_from_destination = TRUE;
        ...
        if (motion->mobility_filename_provided == FALSE) {
            ...
            WARNING(
                "For qualnet-type motion, the name of the file containing \
mobility information must be provided via the '%s' attribute",
                MOTION_MOBILITY_FILENAME_STRING);
            return ERROR;
        }
    }
    ...
    // if coordinate system is not cartesian, and the center for circular
    // motion was used to define motion, then we convert latitude & longitude
    // to x & y before storing the center coordinates
...
int xml_connection_init(struct connection_class *connection,
    ...
    for (i = 0; attributes[i]; i += 2)
        ...
        } else if (strcmp(attributes[i], CONNECTION_FROM_INTERFACE_STRING) ==
                   0) {
            strncpy(connection->from_interface,
                    attributes[i + 1],
                    MAX_STRING - 1);
            // from_node_provided = TRUE;
        ...
        } else if (strcmp(attributes[i], CONNECTION_TO_INTERFACE_STRING) ==
                   0) {
            strncpy(
                connection->to_interface, attributes[i + 1], MAX_STRING - 1);
            // to_node_provided = TRUE;
        } else if (strcmp(attributes[i],
        ...
        } else if (strcmp(attributes[i], CONNECTION_PACKET_SIZE_STRING) ==
                   0) {
            long_int_result = long_int_value(attributes[i + 1]);
            if (long_int_result == LONG_MIN)
                return ERROR;
...
'int' range ('%ld')",
            ...
            if (connection->packet_size < 0) {
                WARNING(
                    "Connection attribute '%s' must be a positive integer. \
Ignored negative value ('%s')",
                    CONNECTION_PACKET_SIZE_STRING,
                    attributes[i + 1]);
            ...
            if (strcmp(attributes[i + 1],
                       CONNECTION_STANDARD_802_11B_STRING) == 0) {
                if (connection_init_standard(connection, WLAN_802_11B) ==
                    ERROR)
                    return ERROR;
            } else if (strcmp(attributes[i + 1],
                              CONNECTION_STANDARD_802_11G_STRING) == 0) {
                if (connection_init_standard(connection, WLAN_802_11G) ==
                    ERROR)
                    return ERROR;
            } else if (strcmp(attributes[i + 1],
                              CONNECTION_STANDARD_802_11A_STRING) == 0) {
                if (connection_init_standard(connection, WLAN_802_11A) ==
                    ERROR)
                    return ERROR;
            } else if (strcmp(attributes[i + 1],
                              CONNECTION_STANDARD_ETH_10_STRING) == 0) {
                if (connection_init_standard(connection, ETHERNET_10) ==
                    ERROR)
                    return ERROR;
            } else if (strcmp(attributes[i + 1],
                              CONNECTION_STANDARD_ETH_100_STRING) == 0) {
                if (connection_init_standard(connection, ETHERNET_100) ==
                    ERROR)
                    return ERROR;
            } else if (strcmp(attributes[i + 1],
                              CONNECTION_STANDARD_ETH_1000_STRING) == 0) {
                if (connection_init_standard(connection, ETHERNET_1000) ==
                    ERROR)
                    return ERROR;
            } else if (strcmp(attributes[i + 1],
                ...
                if (connection_init_standard(connection, WIMAX_802_16) ==
                    ERROR)
                    return ERROR;
            } else {
                WARNING(
...
standard",
                ...
                connection->adaptive_operating_rate = TRUE;
                              ...
                              CONNECTION_RATE_1MBPS_STRING) == 0) {
...
not supported for this standard",
                    ...
                    return ERROR;
                              ...
                              CONNECTION_RATE_2MBPS_STRING) == 0) {
                connection->adaptive_operating_rate = FALSE;
...
not supported for this standard",
                    ...
                    return ERROR;
                              ...
                              CONNECTION_RATE_5MBPS_STRING) == 0) {
                connection->adaptive_operating_rate = FALSE;
...
not supported for this standard",
                    ...
                    return ERROR;
                              ...
                              CONNECTION_RATE_6MBPS_STRING) == 0) {
                connection->adaptive_operating_rate = FALSE;
...
not supported for this standard",
                    ...
                    return ERROR;
                              ...
                              CONNECTION_RATE_9MBPS_STRING) == 0) {
                connection->adaptive_operating_rate = FALSE;
...
Ignored invalid value ('%s')",
                ...
                return ERROR;
                          ...
                          CONNECTION_RTS_CTS_THRESHOLD_STRING) == 0) {
            long_int_result = long_int_value(attributes[i + 1]);
            if (long_int_result == LONG_MIN)
                return ERROR;
            else if ((long_int_result >= INT_MIN) &&
                ...
                WARNING(
                    "Connection attribute '%s' has a value outside the \
'int' range ('%ld')",
                    CONNECTION_RTS_CTS_THRESHOLD_STRING,
                    long_int_result);
                return ERROR;
            }
            ...
            if (connection->RTS_CTS_threshold < 0 ||
                ...
                WARNING(
                    "Connection attribute '%s' must be between 0 and %d. \
Ignored invalid value ('%s')",
                    CONNECTION_RTS_CTS_THRESHOLD_STRING,
                    MAX_PSDU_SIZE_AG,
                    attributes[i + 1]);
            ...
            if (connection->bandwidth < 0 || connection->bandwidth > 1e9) {
                WARNING(
                    "Connection attribute '%s' must be between 0 and 10^9 bps.\
 Ignored invalid value ('%s')",
                    CONNECTION_BANDWIDTH_STRING,
                    attributes[i + 1]);
            ...
            if (connection->loss_rate < 0 || connection->loss_rate > 1) {
                WARNING(
                    "Connection attribute '%s' must be between 0 and 1. \
Ignored invalid value ('%s')",
                    CONNECTION_LOSS_RATE_STRING,
                    attributes[i + 1]);
                return ERROR;
            } else
            ...
            if (connection->delay < 0) {
                WARNING(
                    "Connection attribute '%s' must be positive. Ignored \
invalid value ('%s')",
                    CONNECTION_DELAY_STRING,
                    attributes[i + 1]);
                return ERROR;
            } else {
                connection->variable_delay =
                    connection->delay; // WHY????????????
...
invalid value ('%s')",
            ...
            if (strcmp(attributes[i + 1], TRUE_STRING) == 0)
                ...
                WARNING(
                    "Connection attribute '%s' must be either '%s' or \
...
int xml_fixed_deltaQ_init(struct fixed_deltaQ_class *fixed_deltaQ,
                          const char **attributes) {
    int i;
    ...
    // make sure all parameters are initialized to default values
    fixed_deltaQ_init(fixed_deltaQ,
                      DEFAULT_FIXED_DELTAQ_START_TIME,
                      DEFAULT_FIXED_DELTAQ_END_TIME,
                      DEFAULT_FIXED_DELTAQ_BANDWIDTH,
                      DEFAULT_FIXED_DELTAQ_LOSS_RATE,
                      DEFAULT_FIXED_DELTAQ_DELAY,
                      DEFAULT_FIXED_DELTAQ_JITTER);
    ...
    for (i = 0; attributes[i]; i += 2)
        if (strcmp(attributes[i], XML_SCENARIO_START_TIME_STRING) == 0) {
            ...
            if (xml_scenario->duration < 0) {
                WARNING(
                    "Scenario attribute '%s' must be a positive double. \
Ignored negative value ('%s')",
                    XML_SCENARIO_DURATION_STRING,
                    attributes[i + 1]);
                xml_scenario->duration = 0;
                return ERROR;
            }
            ...
            if (xml_scenario->step < 0) {
                WARNING(
                    "Scenario attribute '%s' must be a positive double. \
Ignored negative value ('%s')",
                    XML_SCENARIO_STEP_STRING,
                    attributes[i + 1]);
                xml_scenario->step = 0.5;
                return ERROR;
            }
        } else if (strcmp(attributes[i],
                          XML_SCENARIO_MOTION_STEP_DIVIDER_STRING) == 0) {
            double_result = double_value(attributes[i + 1]);
            if (double_result == -HUGE_VAL)
                return ERROR;
            else
            ...
            if (xml_scenario->step < 0) {
                WARNING(
                    "Scenario attribute '%s' must be a positive double. \
Ignored negative value ('%s')",
                    XML_SCENARIO_MOTION_STEP_DIVIDER_STRING,
                    attributes[i + 1]);
                xml_scenario->motion_step_divider = 1.0;
                return ERROR;
            }
        } else if (strcmp(attributes[i], XML_SCENARIO_COORD_SYST_STRING) ==
                   0) {
            if (strcmp(attributes[i + 1],
                ...
                WARNING(
                    "Scenario attribute '%s' is assigned an invalid value\
('%s')",
                    XML_SCENARIO_COORD_SYST_STRING,
                    attributes[i + 1]);
                return ERROR;
            }
        } else if (strcmp(attributes[i],
                          XML_SCENARIO_JPGIS_FILENAME_STRING) == 0) {
            strncpy(xml_scenario->jpgis_filename,
                    attributes[i + 1],
                    MAX_STRING - 1);
...
void xml_scenario_print(struct xml_scenario_class *xml_scenario) {
    ...
    printf("\tstart_time=%.2f duration=%.2f step=%.2f\n",
           xml_scenario->start_time,
           xml_scenario->duration,
           xml_scenario->step);
...
static void XMLCALL start_element(void *data,
    ...
    if (xml_depth == 0)
        ...
        if (strcmp(element, XML_SCENARIO_STRING) == 0) {
            ...
            stack_push(
                &(xml_scenario->stack), xml_scenario, ELEMENT_XML_SCENARIO);
            if (xml_scenario_init(xml_scenario, attributes) == ERROR)
                xml_scenario->xml_parse_error = TRUE;
        } else {
            WARNING(
                "Expected top-level entity '%s' not found. XML file \
is not of 'QOMET definition' type!",
                XML_SCENARIO_STRING);
            xml_scenario->xml_parse_error = TRUE;
        }
    ...
    else if (strcmp(element, NODE_STRING) == 0) // check if node
    {
        ...
        if (xml_node_init(&node,
                          attributes,
                          xml_scenario->cartesian_coord_syst) == ERROR)
            xml_scenario->xml_parse_error = TRUE;
        else if (node_check_valid(
                     scenario->nodes, scenario->node_number, &node) == TRUE) {
    ...
    } else if (strcmp(element, INTERFACE_STRING) == 0) // check if interface
    {
        ...
        if (xml_interface_init(&interface, attributes) == ERROR)
            ...
            /*
               ...
               &node) == TRUE)
               {
               if ((element_ptr = scenario_add_node (scenario, &node)) ==
               NULL) xml_scenario->xml_parse_error = TRUE;
               }
               else
               xml_scenario->xml_parse_error = TRUE;
             */
    ...
    } else if (strcmp(element, OBJECT_STRING) ==
               0) // check if topology object
    {
        DEBUG("Object element found");

        if (xml_object_init(&object,
                            attributes,
                            xml_scenario->cartesian_coord_syst) == ERROR)
            xml_scenario->xml_parse_error = TRUE;
        else if (object_check_valid(scenario->objects,
                                    scenario->object_number,
                                    &object,
                                    xml_scenario->jpgis_filename_provided) ==
                 TRUE) {
            if ((element_ptr = scenario_add_object(scenario, &object)) ==
                NULL)
                xml_scenario->xml_parse_error = TRUE;
        } else
            xml_scenario->xml_parse_error = TRUE;
    ...
    } else if (strcmp(element, COORDINATE_STRING) == 0) // check if coordinate
    {
        ...
        stack_push(&(xml_scenario->stack),
                   &(xml_scenario->coordinate),
                   ELEMENT_COORDINATE);
    } else if (strcmp(element, ENVIRONMENT_STRING) ==
               0) // check if environment
    {
        DEBUG("Environment element found");

        if (xml_environment_init(&environment, attributes) == ERROR)
            xml_scenario->xml_parse_error = TRUE;
        else if (environment_check_valid(scenario->environments,
                                         scenario->environment_number,
                                         &environment) == TRUE) {
            if ((element_ptr = scenario_add_environment(
    ...
    } else if (strcmp(element, FIXED_DELTAQ_STRING) ==
               0) // check if fixed_deltaQ
    {
        ...
        if (xml_fixed_deltaQ_init(&fixed_deltaQ, attributes) == ERROR)
            ...
            if (stack_top_element != NULL &&
                ...
                WARNING(
                    "fixed_deltaQ element not embedded in a \
connection element");
                xml_scenario->xml_parse_error = TRUE;
            } else {
                struct connection_class *connection =
...
static void XMLCALL end_element(void *data, const char *element) {
    ...
    // further processing
    if (xml_scenario->xml_parse_error == TRUE) return;
    ...
    switch (stack_element->element_type) {
        ...
        case ELEMENT_COORDINATE: {
            ...
            if (count < 2) {
                WARNING(
                    "At least 2 coordinates must be provided. \
Only height/z coordinate are optional");
            ...
            if (stack_top_element != NULL &&
                ...
                WARNING(
                    "Coordinate element not embedded in an object element");
                xml_scenario->xml_parse_error = TRUE;
            } else {
                struct object_class *object =
                ...
                if (object->vertex_number < MAX_VERTICES) {
                    ...
                    WARNING(
                        "Maximum number of vertices in object exceeded (%d)",
                        MAX_VERTICES);
                    xml_scenario->xml_parse_error = TRUE;
                }
            }
...
int xml_scenario_parse(FILE *scenario_file,
    ...
    do {
        // read one buffer
        buffer_length =
            (int) fread(xml_buffer, 1, BUFFER_SIZE, scenario_file);
        ...
        if (XML_Parse(xml_parser, xml_buffer, buffer_length, parsing_done) ==
            XML_STATUS_ERROR) {
            WARNING(
                "XML syntax-level parse error in input file at line "
                "#%" XML_FMT_INT_MOD "u: \"%s\"",
