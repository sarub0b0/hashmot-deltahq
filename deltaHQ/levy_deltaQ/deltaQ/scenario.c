            ...
            if (scenario->environments[env_i].name_hash ==
                ...
                if (strcmp(scenario->environments[env_i].name,
                    ...
                    /*
                       ...
                       cannot be used to \ define multiple connections",
                       connection-> through_environment); return NULL;
                       }
                       else
                       {
                     */
        ...
        for (node_i = 0; node_i < scenario->node_number; node_i++)
            ...
            if (strcmp(connection->from_node,
                ...
                if (scenario->connection_number < MAX_CONNECTIONS) {
                    ...
                    // to create and add an environment for this connection;
...
no merging necessary",
    ...
    for (connection_i = 0; connection_i < scenario->connection_number;
        ...
        /* Commented out to speed up processing for large scenarios
           fprintf (stderr, "Initializing indexes for connection %d (out of
           %d)\r", connection_i, scenario->connection_number);
         */
        ...
        for (connection_i = 0; connection_i < scenario->connection_number;
            ...
            do {
                ...
                if (num_iterations >= MAXIMUM_PRECOMPUTE) {
                    ...
                    DEBUG(
                        "Maximum number of iterations (%d) was \
reached during precomputation phase without reaching steady state",
        ...
        DEBUG(
            "* Connection validation and initialization done (%d \
connections)      \n",
            scenario->connection_number);
    }
...
int scenario_deltaQ(struct scenario_class *scenario, double current_time) {
    ...
    struct calculation_class *calc;

    calc = &scenario->calculations;

    for (int i = 0; i < calc->calculation_number; i++) {
        connection = &(
            scenario
                ->connections[calc->connection_indexes[i]]);
        ...
        if (connection->fixed_deltaQ_number > 0) {
            fixed_deltaQ =
                &(connection->fixed_deltaQs[connection->fixed_deltaQ_crt]);
            ...
            if (fixed_deltaQ->end_time <= current_time)
                if (connection->fixed_deltaQ_crt <
                    ...
                    fixed_deltaQ =
                        &(connection
                              ->fixed_deltaQs[connection->fixed_deltaQ_crt]);
                }

            DEBUG(
                "fixed_deltaQ: fixed_deltaQ_number=%d fixed_deltaQ_crt=%d \
current_time=%.2f",
        ...
        if (connection_deltaQ(connection, scenario, &deltaQ_changed) == ERROR)
            return ERROR;
    }
