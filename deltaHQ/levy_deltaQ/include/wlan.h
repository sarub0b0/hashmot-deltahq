...
// the standard specifies attenuation at 11 MHz on left and right of center
// frequency! Formula: P_dBm = -174 + 10*log10(delta_f) [Wikipedia:
// Johnson-Nyquist noise] with delta_f expressed in Hertz Note: 802.11a uses
// the same bandwidth => same noise level
...
struct parameters_802_11b {
    ...
    float
        model2_a[B_RATES_NUMBER]; // parameter 'a' of the exponential fitting
    float
        model2_b[B_RATES_NUMBER]; // parameter 'b' of the exponential fitting
};
