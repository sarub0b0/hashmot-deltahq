...
void LSH::Init(const vector<NodeParam> &nodes) {
...
vector<int> LSH::GetNeighbor(const NodeParam &node) {
    vector<int> neighbor;

    for (auto &&lsh : lsh_) {
        if (lsh->IsSameRadius(node.radius)) {
            array<float, 2> p{node.x, node.y};
...
void LSH::SendDeltaHQ(vector<int> &neighbor,
